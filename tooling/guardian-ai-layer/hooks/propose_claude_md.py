...
Three guards keep it well-behaved:
  * Recursion guard — the reflector spawns a headless `claude` whose own Stop
    hook lands right back here; GUARDIAN_AILAYER_REFLECT_LOCK makes that no-op.
...
_LOCK_ENV = "GUARDIAN_AILAYER_REFLECT_LOCK"
...
def _spawn_reflector(reflector: Path, root: Path) -> bool:
    ...
    try:
        ...
        print(f"[guardian-ai-layer hook] could not start reflector: {exc}", file=sys.stderr)
        return False
    return True
...
def main() -> int:
    ...
    if not reflector.is_file():
        print(f"[guardian-ai-layer hook] {_REFLECTOR} missing — skipped", file=sys.stderr)
        return 0
    ...
    print(
        f"[guardian-ai-layer hook] {len(areas)} area(s) changed "
