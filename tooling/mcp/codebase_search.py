"""Codebase-search MCP server for Phantom — AST-based structured search.
...
try:
    ...
    raise SystemExit(
        "phantom-codebase-search requires `mcp[cli]` — install with "
        "`pip install 'mcp[cli]'`."
    ) from exc
...
mcp = FastMCP("phantom-codebase-search")
