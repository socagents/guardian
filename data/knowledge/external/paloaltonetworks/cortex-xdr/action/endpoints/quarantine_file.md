...
**MCP tool**: `xdr_endpoints_quarantine_file`
**Guardian connector**: `cortex-xdr` (per-instance: see Tools tab on `/connectors/cortex-xdr-<instance>`)

**Official docs**: https://docs.paloaltonetworks.com/cortex/cortex-xdr/cortex-xdr-api/cortex-xdr-apis/response-actions/quarantine-files.html
...
| `incident_id` | `str` | `None` | When included in the request, the Quarantine File action will appear in the Cortex XDR Incident View Timeline tab. |
...
- Generated from `ebarti/cortex-xdr-client`'s `quarantine_file` wrapper — see https://github.com/ebarti/cortex-xdr-client for the authoritative Python implementation.
...
- Guardian tool: `bundles/spark/connectors/cortex-xdr/src/connector.py` → `xdr_endpoints_quarantine_file`
