...
`/api/v1/audit` endpoint via the URL in `GUARDIAN_AUDIT_URL`,
...
logged at WARN level (visible via `docker logs guardian-connector-X`)
and dropped.
...
_AUDIT_URL_ENV = "GUARDIAN_AUDIT_URL"
...
class _AuditForwarder:
    ...
    async def start(self) -> None:
        ...
        logger.info(
            ...
            self._url or "(disabled — no GUARDIAN_AUDIT_URL)",
            _QUEUE_CAP,
        )
