...
### Phase 4.5 — Update Parent Issue with Sub-Issue List

After ALL sub-issues are created, update the parent issue body to list them.
This is critical — the auto-close workflow scans the parent body for `#NNN`
references to determine when all children are done. Without this step, parent
issues will never auto-close.

Use `gh issue edit` to replace the `## Sub-Issues` placeholder with the actual
sub-issue list:

```
gh issue edit <PARENT_NUMBER> --repo "$REPO" --body "$(
  # Rebuild the parent body with the Sub-Issues section filled in
)"
```

The Sub-Issues section should list each sub-issue with its number:

```markdown
## Sub-Issues

- #101: Implement config loading
- #102: Add gRPC server setup
- #103: Wire health check endpoint
```
...
- Common dependency patterns:
  ...
  - **Same-package serialization**: issues that create or modify files in the
    same package/directory (e.g., `providers/__init__.py`, `providers/types.py`,
    `providers/errors.py`) MUST have `Depends on:` between them. Concurrent PRs
    touching the same directory cause merge conflicts, wasting agent tokens on
    re-implementation. Chain them sequentially even if logically independent.
