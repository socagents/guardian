#!/usr/bin/env bash
# Guardian — operator restore utility.
#
# Reverse of backup_guardian.sh. Reads a backup archive produced by
...
#
# Usage:
#   scripts/restore_guardian.sh ARCHIVE [--force]
...
ARCHIVE=""
FORCE=0
DATA_VOLUME="guardian_mcp_data"
SKILLS_VOLUME="guardian_mcp_skills"
RUNTIME_DIR=""
...
work_dir="$(mktemp -d -t guardian-restore-XXXXXX)"
...
if [ ! -f "$manifest" ]; then
  printf 'ERROR: backup-manifest.json missing — not a guardian backup.\n' >&2
  exit 3
fi
