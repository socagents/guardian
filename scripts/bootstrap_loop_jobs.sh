...
print(json.dumps({
  ...
  "action": {
    ...
    "message": (
      ...
      "(upsert each IoC with indicator_upsert + link STIX edges with "
      "indicator_relate), scope the blast radius, and — for any TRUE "
      "POSITIVE — draw the attack-chain diagram (generate the SVG per the "
      "svg_attack_chain skill, store it with issue_set_attack_chain) and the "
      "indicator-relations graph (svg_relation_graph skill, stored with "
      "issue_set_relation_graph). Then finish with "
      "issue_update(status='resolved') "
...
print(json.dumps({
  ...
  "action": {
    "type": "prompt",
    "message": (
      "You are the autonomous investigation-judge. Be TERSE and DECISIVE — "
      "do NOT narrate your reasoning or explain what you are about to do; "
      ...
      "skill change needed' and STOP. If one or more are weak, pick the "
      "single WEAKEST, call skills_read('workflows/"
      "xsoar_case_investigation.md') ONCE, then make ONE "
      "skills_update('workflows/xsoar_case_investigation.md', <full new "
      "content>) that ADDS a concrete instruction strengthening ONLY that "
      "dimension. HARD CONSTRAINTS: preserve all 6 steps + every heading; "
      "additive only, never delete a step; <=~20 added lines; touch no "
      "other skill; AT MOST one skills_update. Finish with a one-line "
      "tally (e.g. 'VERDICT 5/5, MITRE 2/5 weak — strengthened MITRE step') "
      "and what you changed, or that investigations are healthy."
    ),
  },
}))
PY
