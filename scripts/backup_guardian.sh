#!/usr/bin/env bash
# Guardian — operator backup utility.
#
# Captures the persistent state that makes a Guardian deploy
# recoverable on a different host (or after a `docker volume rm`):
#
#   1. guardian_mcp_data volume — sqlite stores: audit log, instance
#      store, secret paths, KB, sessions, memory, jobs.
#   2. guardian_mcp_skills volume — skills library (markdown).
#   3. ./.guardian-agent/ bind-mount directory (if present) — setup
...
#
# Usage:
#   scripts/backup_guardian.sh [--output DIR] [--label LABEL]
#                             [--data-volume NAME] [--skills-volume NAME]
#                             [--runtime-dir PATH]
#
# Output:
#   <DIR>/guardian-backup-<LABEL>-<UTC-stamp>.tar.gz
#
#   The tarball contains:
#     backup-manifest.json     — provenance + sha256 of inner artifacts
#     data.tar.gz              — guardian_mcp_data contents
#     skills.tar.gz            — guardian_mcp_skills contents (if present)
#     runtime/                 — copy of ./.guardian-agent/ (if present)
#
# Exit codes:
#   0 = backup complete
...
if [ -z "$DATA_VOLUME" ]; then
  DATA_VOLUME="$(discover_volume guardian_mcp_data)"
fi
if [ -z "$SKILLS_VOLUME" ]; then
  SKILLS_VOLUME="$(discover_volume guardian_mcp_skills)"
fi

if [ -z "$DATA_VOLUME" ]; then
  printf 'ERROR: could not find guardian_mcp_data volume.\n' >&2
...
archive_name="guardian-backup-${LABEL}-${stamp}.tar.gz"
archive_path="${OUTPUT_DIR}/${archive_name}"
work_dir="$(mktemp -d -t guardian-backup-XXXXXX)"
...
# Manifest describes what's in the backup so restore_guardian.sh
...
printf '  scripts/restore_guardian.sh %s\n' "$archive_path"

# KEK reminder. The secrets directory inside `guardian_mcp_data` is
# AES-GCM ciphertext when GUARDIAN_SECRET_KEK is set. The KEK lives
...
if [ -f "$ROOT_DIR/.env" ] && grep -q '^GUARDIAN_SECRET_KEK=' "$ROOT_DIR/.env" 2>/dev/null; then
  printf '\n'
  printf '  ⚠  GUARDIAN_SECRET_KEK is set — secrets in this backup are\n'
