...
- `xsiam/` — Cortex XSIAM integration over the Cortex public API (`/public_api/v1`). 56 tools: investigation (XQL, incidents, alerts, issues, assets, audit, datamodel) + EDR response (endpoint isolate/scan/quarantine, script run, IOC + hash blocklist). Auth is the api_id (`x-xdr-auth-id`) + api_key (`Authorization`) pair. Write tools are approval-gated; `xsiam.remove_lookup_data` is manifest-denied.
