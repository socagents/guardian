...
Each connector ships as its own image at customer release time. The agent dispatches to per-instance containers via HTTP, not in-process Python — crash isolation + independent versioning. `phantom-updater` (see [`../../../updater/CLAUDE.md`](../../../updater/CLAUDE.md)) starts/stops the per-instance containers.
...
2. **`src/connector.py`** — the connector's entry point. Inherits from the `phantom-connector-runtime` base (`../../../phantom-connector-runtime/`).
3. **`src/<tool>.py` files** — per-tool implementations.
4. **`tests/`** — connector-local pytest suite.
5. **`Dockerfile`** — `FROM ghcr.io/kite-production/phantom-connector-runtime:latest`. CI builds + tags each connector image at customer release time.

## Runtime style

`runtimeMapping.style: container` is the only supported style as of v0.5.0. Each instance = one Docker container, managed by `phantom-updater`. The agent's tool dispatcher proxies calls over MCP-over-HTTP to `http://phantom-connector-<id>-<name>:9000`.
...
- YAML MUST declare an `image` field — the OCI reference to the operator's pre-published connector container. phantom-updater pulls this image when instances are created.
