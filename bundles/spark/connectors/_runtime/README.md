# Guardian connector runtime contract — reference skeleton

> **Audience**: bundle authors writing a new Guardian connector for the
> v0.2 architecture. Read first; copy second.
>
> **See also**: `docs/spec-per-instance-connector-containers.md` for the
> architectural rationale.

A Guardian connector running under `runtimeMapping.style: container`
...
1. **Be `FROM guardian-connector-runtime:<version>`**. The base image
...
- **`/health` endpoint** — runtime adds it. Used by Docker healthcheck
  + guardian-updater readiness + agent-side proxy probes.
...
- **Tool name prefix stripping** — `guardian_<id>_<tool>` becomes `<tool>`
...
├── Dockerfile                     ← FROM guardian-connector-runtime + COPY src
└── src/
    ├── __init__.py
...
> but it cannot be built as `guardian-connector-_runtime:test` — Docker
...
## Validation checklist before shipping a new connector
  ...
  - [ ] `Dockerfile` is `FROM ghcr.io/kite-production/guardian-connector-runtime:<version>`
  ...
  - [ ] Per-connector Python deps are pinned in your Dockerfile, NOT
        in `guardian-connector-runtime/requirements.txt`
  - [ ] Manifest's `toolConnectors[]` has your entry
  - [ ] release.yml has a "Build or retag guardian-connector-<id>" step
  - [ ] `docker build -f bundles/spark/connectors/<id>/Dockerfile .` succeeds
  - [ ] `docker run --rm --entrypoint python guardian-connector-<id>:test -c
