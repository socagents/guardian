...
- `knowledge_search` / `knowledge_list` — **Guardian's bundled knowledge bases**, the **internal, instant, first-choice** reference (consult BEFORE cortex-docs/web). READ-ONLY — there is no write tool (distinct from `memory_*`, the agent's mutable org memory). Two KBs today:
  ...
  - **`mitre-atlas`** — MITRE ATLAS, the ATT&CK-style framework for attacks on **AI/ML systems** (prompt injection, model evasion, data poisoning, agent hijacking): 170 techniques + 57 real-world AI-incident case studies. Search this when the case involves an **AI/LLM target or an AI-enabled attack** (e.g. `AML.T0051` LLM Prompt Injection). To search all KBs at once, omit `kb_name`; to scope, pass `kb_name`. See § Step 3.
