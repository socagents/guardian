...
Hit the operator's example-query knowledge base via the agent's existing `knowledge_search` tool family (NOT a cortex-docs tool — this is a separate KB). Aim for 5 hits, top similarity. Each hit is a previously-validated XQL query with metadata about what it did.

If the operator's KB has no relevant entries, skip directly to step 4 — the cortex-docs lookup alone is usable, just less idiomatic.
...
| `xsiam_get_datasets` returns `{"error": "xsiam instance has no api_key (Authorization header) configured"}` or similar XSIAM connector error | The operator hasn't installed/configured an XSIAM provider, OR the configured instance is missing the API key | **Do NOT retry via other XSIAM tools** (they will all fail the same way). Ask the operator directly for the dataset name (e.g. `cisco_esa_raw`, `xdr_data`) and proceed with cortex-docs/xql_lookup for syntax. The skill is designed to work end-to-end without XSIAM access — the operator's KB and cortex-docs cover the authoring path. Only mention XSIAM tools as a side note: *"If you'd like the agent to discover datasets automatically, configure an XSIAM provider in /providers."* (v0.5.59+ — legacy `papiAuthHeader` error message text still possible on instances created pre-rename.) |
| `xsiam_get_xql_doc` returns `{"error": "xql_doc.md not found"}` | This tool was removed in v0.3.5 (it was a legacy bundled offline doc that was never shipped). | **Don't call this tool.** Use `cortex-docs/xql_lookup` instead (per the workflow above) — it's universal-access and supersedes the legacy bundled doc. |
