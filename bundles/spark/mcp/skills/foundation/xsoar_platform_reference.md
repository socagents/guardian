...
| **Indicator store** | XSOAR's IoC database — IPs, domains, URLs, file hashes, CVEs, emails, each with a reputation/DBotScore + relationships. Search it with `xsoar_search_indicators`; get a live verdict for one IoC with `xsoar_enrich_indicator`. **Write** to it with `xsoar_create_indicator` (add an IoC, optionally with a verdict + tags) and `xsoar_update_indicator` (persist a verdict back: `score` 0-3 = Unknown/Good/Suspicious/Bad, sets `manualScore`, plus tags) — this is how the agent's IoC verdicts stick tenant-wide, not just on one incident. Both are REST-path (`/indicator/create`, `/indicator/edit`) — no `playground_id`, both generations. |
...
| **Playbook** | An automation workflow. **Discover** what the tenant has with `xsoar_list_playbooks` (catalog; filter `name:`/`tag:`/`system:T\|F`) and read one's inputs + tasks with `xsoar_get_playbook` (by id or name) — do this BEFORE recommending or running a playbook so you pick one that exists and know its inputs. Run one on a case with `xsoar_run_playbook` (by name, in the case's own War Room — no `playground_id`); import one with `xsoar_import_playbook`. `list_playbooks`/`get_playbook` are REST-path tools (`/playbook/search`) — no `playground_id`, both generations. |
| **Job (scheduled)** | Automation that runs a playbook on a schedule — a cron-recurring or one-off job that creates an incident of a given type and runs its playbook. Manage with `xsoar_list_jobs` (catalog), `xsoar_get_job` (one job), `xsoar_create_job` (schedule one: recurring needs `cron`, one-time needs `start_date`), `xsoar_delete_job`. All REST-path (`/jobs/search`, `/jobs`, `DELETE /jobs/{id}`) — no `playground_id`. NOTE: XSOAR v6 has no REST "run a job now" endpoint — to run immediately, create the incident directly (`xsoar_create_incident`) + `xsoar_run_playbook`, or rely on the schedule. |
...
- **For "top / by reputation", put the reputation in the QUERY — `reputation:Bad` (or `verdict:Malicious` / `verdict:Suspicious`), optionally with a type, e.g. `type:IP and reputation:Bad`.** That returns the scored indicators already filtered. Do NOT pull an unfiltered `type:X` page and expect it sorted by score — XSOAR has no server-side reputation sort, so an unfiltered page is in store order (often newest-first, unscored), NOT worst-first. Filter at the query instead. There is **no `score:N` query field** — use `reputation:` / `verdict:`. And never fall back to `!findIndicators` / `!py` / cortex-docs: `search_indicators` with the right query is the complete answer.
