...
Query params for GET /api/v1/audit:
    ...
    target          exact match, e.g. "tool:xsoar.list_incidents"
...
def register_audit_routes(mcp: FastMCP, audit: SqliteAuditLog) -> None:
    ...
    async def write_audit(request: Request) -> JSONResponse:
        ...
        # Next.js middleware stamped (X-Guardian-Actor: apikey:<id> |
        # user:operator). Read the HEADER directly (not get_current_actor(),
        # which would pick up the MCP's ambient "system" default) so the
        # absent-header case preserves the prior "user:operator" default
        # for these operator-initiated audit writes.
        actor_token = set_current_actor(
            request.headers.get("X-Guardian-Actor") or "user:operator"
        )
        try:
            try:
