...
Cortex XSIAM Troy NGFW Remote Access Activity [Front] - Custom XQL Widget. Queries the `panw_ngfw_traffic_raw` dataset directly, filtered on `app_sub_category = "remote-access"`. Uses stages: `filter`, `alter`, `comp`, `fields`, `sort`, `limit`.
