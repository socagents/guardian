...
Troy Playbook Automation - Custom XQL Widget. Queries the `xsiam_playbookmetrics_raw` dataset directly, filtered on `playbookId != ""`. Uses stages: `alter`, `dedup`, `filter`, `join`, `bin`, `windowcomp`.
