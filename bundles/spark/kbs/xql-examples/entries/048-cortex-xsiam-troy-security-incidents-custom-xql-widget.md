...
Cortex XSIAM Troy Security Incidents - Custom XQL Widget. Queries the `incidents` dataset directly, filtered on `starred = 1`. Uses stages: `filter`, `fields`, `join`, `alter`, `comp`, `sort`.
