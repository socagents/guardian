...
Cortex XSIAM Troy SOC Value Metrics - Custom XQL Widget. Queries the `incidents` dataset directly, filtered on `timestamp_diff(current_time(),creation_time, "DAY") <= 1`. Uses stages: `fields`, `filter`, `join`, `comp`, `alter`, `view`.
