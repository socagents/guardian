# Phantom AI Layer — Article Alignment & Adoption Status
...
This file documents which article components Phantom has adopted, and where each one lives. Updated as adoption progresses.
...
|---|---|---|---|
| **1 — Foundation** | Lean root CLAUDE.md, subdirectory CLAUDE.md hierarchy, CODEBASE_MAP.md, .claudeignore, AI-LAYER.md (this file) | **✓ Done** | [#77](https://github.com/kite-production/phantom/issues/77) (closed) |
| **2 — Infrastructure** | `.claude/settings.json`, explorer subagent, SessionStart + Stop hooks, path-scoped skills, codebase-search MCP | **✓ Done** | [#78](https://github.com/kite-production/phantom/issues/78) (closed) |
| **3 — Governance** | phantom-ai-layer plugin, marketplace.json, validator harness, CICD review cadence | **✓ Done** | [#79](https://github.com/kite-production/phantom/issues/79) (closed) |
| **4 — Phantom product (optional)** | Harness validation observability page (`/observability/harness`) | P3, deferred | [#80](https://github.com/kite-production/phantom/issues/80) |

---

## The extension points — article → artifact → status

| Extension point | What the article says | Phantom artifact | Status |
|---|---|---|---|
| **CLAUDE.md files** | Loaded first; lean root, subdirectory files load additively as Claude walks the tree | Root [`CLAUDE.md`](CLAUDE.md) (lean, ~540 lines down from 901) + 6 subdirectory files: [`mcp/agent/`](mcp/agent/CLAUDE.md), [`bundles/spark/mcp/`](bundles/spark/mcp/CLAUDE.md), [`bundles/spark/connectors/`](bundles/spark/connectors/CLAUDE.md), [`xlog/`](xlog/CLAUDE.md), [`installer/`](installer/CLAUDE.md), [`updater/`](updater/CLAUDE.md) | **✓ Phase 1** |
...
| **Skills** | On-demand expertise, progressive disclosure, scoped to specific paths | [`.claude/skills/`](.claude/skills/) with 5 path-scoped coding skills: `connector-add`, `mcp-tool-add`, `release-tag-flow`, `help-page-update`, `agent-page-add`. Each carries `paths:` frontmatter (validator asserts). NB: Phantom's RUNTIME skills (`bundles/spark/skills/`) already follow this pattern for the chat agent — the AI Layer brings the same pattern to the CODING-agent layer | **✓ Phase 2** |
| **Plugins** | Bundle skills/hooks/MCP into installable packages, distribute via a marketplace | [`tooling/phantom-ai-layer/`](tooling/phantom-ai-layer/) + [`tooling/.claude-plugin/marketplace.json`](tooling/.claude-plugin/marketplace.json). Bundles ONLY the repo-agnostic parts (explorer subagent, propose/reflect hooks, codebase-search MCP, generic scoped-tests skill). Phantom-specific skills intentionally stay in `.claude/skills/`. NB: Phantom's RUNTIME has the connector marketplace (analog) — Phase 3 brought the pattern to the coding-agent layer | **✓ Phase 3** |
...
| **Subagents** | Split exploration from editing — read-only mapper, separate context window | [`.claude/agents/phantom-explorer.md`](.claude/agents/phantom-explorer.md) — tools: `Read, Grep, Glob` ONLY (no Write/Edit/Bash). Validator asserts no write tools | **✓ Phase 2** |
...
- **Initialized in subdirectories**, not just the root — Claude walks up the tree, so local context never gets lost. **✓ Phase 1** (6 subdirectory files at `mcp/agent/`, `bundles/spark/mcp/`, `bundles/spark/connectors/`, `xlog/`, `installer/`, `updater/`).
...
- **Scoped test commands** — each subdirectory `CLAUDE.md` documents its local pytest + tsc + lint snippet. **✓ Phase 1** (root's full pre-deploy gate stays in root; per-directory commands in each subdirectory file). Plus a portable `scoped-tests` skill in the plugin payload at `tooling/phantom-ai-layer/skills/scoped-tests/`. **✓ Phase 3.**
...
## Getting started — the article's 4 phases mapped to Phantom

| Phase | Article | Phantom artifacts |
|---|---|---|
...
| **Governance** | review requirements, DRI, approvals | `tooling/phantom-ai-layer/` plugin + marketplace.json, `tooling/validate/validate_all.py`, `docs/CICD.md` § AI Layer review cadence. |
| **Scale** | expand skills/plugins, iterate CLAUDE.md, periodic review | The `Stop` hook + the 3–6 month cadence (Phase 3 mechanics). Long-term: the Phantom-product harness validation page ([#80](https://github.com/kite-production/phantom/issues/80)). |

---

## Phantom-runtime vs coding-agent layer — both exist

Phantom is itself an agentic platform. Some article concepts map cleanly onto Phantom's runtime as well as the coding agent:

| Concept | Phantom RUNTIME (the product) | CODING-AGENT layer (this AI Layer) |
|---|---|---|
| Skills | `bundles/spark/skills/` — used by the chat agent at runtime | `.claude/skills/` — used by Claude Code when editing the repo |
| Hooks | Phantom's hooks framework (settings/hooks page) — fires on chat events | `.claude/hooks/` — fires on Claude Code session events |
| Subagents | Phantom's `/agents` page — runtime agent definitions | `.claude/agents/phantom-explorer.md` — read-only mapper for coding sessions |
| MCP servers | The ~80-tool embedded MCP at `bundles/spark/mcp/` — drives the chat agent | `tooling/mcp/codebase_search.py` — symbol search for coding sessions |
...
Phase 4 ([#80](https://github.com/kite-production/phantom/issues/80)) optionally mirrors the validator pattern INTO Phantom's runtime — a `/observability/harness` page that surfaces Phantom's own runtime equivalents (skills well-formed, agents runnable, hooks valid, MCP tool count). Marked P3 / optional because most of the data is already surfaced piecemeal on individual pages.

---

## Caveats

- Phantom is **not** a green-field implementation of the article's harness — it's a substantial existing product where the patterns are being layered in incrementally. Phase 1's lean root still carries ~540 lines because the agent-behavior contracts (spec-driven workflow, pre-deploy gate, smoke-test discipline, credential guardrail, release-readiness gate, canonical-state discipline) are genuinely load-bearing repo-wide. The article's "lean root" guidance presumes simpler base contracts; Phantom's enterprise discipline keeps the root substantive.
...
- The CODING-agent layer (this file) does NOT replace Phantom's RUNTIME documentation (`/help/architecture`, `/help/user`, `journeys.ts`). Those are operator-facing; the AI Layer is contributor-facing.
...
- `.claude/agents/phantom-explorer.md` has NO write tools
- All hooks compile
...
- [#77 — Phase 1 Foundation](https://github.com/kite-production/phantom/issues/77)
- [#78 — Phase 2 Infrastructure](https://github.com/kite-production/phantom/issues/78)
- [#79 — Phase 3 Governance](https://github.com/kite-production/phantom/issues/79)
- [#80 — Phase 4 P3 Phantom-product harness validation page](https://github.com/kite-production/phantom/issues/80)
