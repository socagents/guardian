...
| **Skills** | On-demand expertise, progressive disclosure, scoped to specific paths | [`.claude/skills/`](.claude/skills/) with 5 path-scoped coding skills: `connector-add`, `mcp-tool-add`, `release-tag-flow`, `help-page-update`, `agent-page-add`. Each carries `paths:` frontmatter (validator asserts). NB: Guardian's RUNTIME skills (`bundles/spark/skills/`) already follow this pattern for the chat agent — the AI Layer brings the same pattern to the CODING-agent layer | **✓ Phase 2** |
