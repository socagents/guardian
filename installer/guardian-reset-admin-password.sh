#!/usr/bin/env bash
# Guardian admin-password reset (v0.5.3+)
#
...
#
#   sudo /opt/guardian/guardian-factory-reset            ← wipe state
#   sudo /opt/guardian/guardian-reset-admin-password     ← reset password
#
...
# guardian-agent image (`/app/cli/reset-admin.mjs`) so the SecretStore
...
#
# Set GUARDIAN_CONTAINER if you renamed the agent container (default:
# guardian_agent). Set GUARDIAN_INSTALL_DIR if your install lives
# somewhere other than /opt/guardian — the script reads .env from
# there to surface a clear error if the stack is misconfigured.

set -euo pipefail

CONTAINER="${GUARDIAN_CONTAINER:-guardian_agent}"
...
if ! docker ps --format '{{.Names}}' | grep -qx "$CONTAINER"; then
  ...
  if [[ -f "$INSTALL_DIR/docker-compose.yml" ]]; then
    ...
    warn "I also don't see $INSTALL_DIR/docker-compose.yml — is Guardian installed?"
    warn "If you renamed the container, set GUARDIAN_CONTAINER=<name> and re-run."
  fi
