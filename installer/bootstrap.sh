#!/usr/bin/env bash
# Guardian — clean-VM bootstrap helper.
#
...
#   2. Prompts for GUARDIAN_VERSION (defaults to "latest" floating tag).
#   3. Prompts for registry credentials (GHCR user + token).
#   4. Auto-generates three secrets with `openssl rand`:
#        MCP_TOKEN, GUARDIAN_SECRET_KEK, GUARDIAN_DEFAULT_ADMIN_PASSWORD
...
#
# Non-interactive mode: set GUARDIAN_NONINTERACTIVE=1 and supply ALL
...
say ""
say "Guardian bootstrap — clean-VM install helper"
say "==========================================="
say ""
...
if [[ -f .env ]]; then
  ...
  warn "(Back up any secrets you need: \`cat .env > /tmp/guardian-env.bak\`)"
  exit 1
fi

# ─── Helpers for prompts ──────────────────────────────────────────────
NONINTERACTIVE="${GUARDIAN_NONINTERACTIVE:-0}"
...
say "1) Image version"
say ""
echo "   Which Guardian version do you want to install?"
...
echo ""
prompt GUARDIAN_VERSION "Version" "latest"
...
echo ""
prompt GUARDIAN_REGISTRY_USER "GHCR username" "kite-deploy-bot"
prompt_secret GUARDIAN_REGISTRY_TOKEN "GHCR token (input hidden)"

if [[ "$GUARDIAN_REGISTRY_TOKEN" == "ghp_REPLACE_WITH_TOKEN_FROM_ONBOARDING" ]] \
   || [[ ${#GUARDIAN_REGISTRY_TOKEN} -lt 30 ]]; then
...
info "Generating GUARDIAN_SECRET_KEK (AES-256 key for SecretStore)…"
GUARDIAN_SECRET_KEK="$(openssl rand -base64 32)"
info "Generating GUARDIAN_DEFAULT_ADMIN_PASSWORD (v0.5.5+ bootstrap admin)…"
...
cat > "$TMP_ENV" <<EOF
# Guardian — generated by bootstrap.sh on $(date -u +%Y-%m-%dT%H:%M:%SZ).
...
GUARDIAN_REGISTRY_USER=$GUARDIAN_REGISTRY_USER
GUARDIAN_REGISTRY_TOKEN=$GUARDIAN_REGISTRY_TOKEN
...
# hardcoded in the guardian-agent image; v0.5.5 moves it here so no
...
echo "     - docker login ghcr.io (using GUARDIAN_REGISTRY_TOKEN)"
echo "     - docker compose pull"
echo "     - docker compose up -d"
echo "     - wait for guardian-agent to become healthy"
echo ""

if [[ "$NONINTERACTIVE" == "1" ]]; then
  RUN_INSTALL="${GUARDIAN_AUTO_INSTALL:-yes}"
else
...
case "${RUN_INSTALL,,}" in
  ...
  *)
    ...
    say ""
    say "   GUARDIAN_VERSION=$GUARDIAN_VERSION"
    say ""
