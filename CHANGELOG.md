...
- **Creating an XSIAM lookup dataset from chat now works instead of hanging.** The connector had a `create_dataset` tool that POSTed to a Cortex endpoint (`xql/add_dataset`) that doesn't exist — the call hung until it timed out. Per the Cortex docs (confirmed via Guardian's own cortex-docs search), there is no REST "create dataset" endpoint: a lookup dataset is **created automatically on the first write** to it. So `create_dataset` is removed and **`add_lookup_data` is now the one tool that both creates and populates** a lookup dataset. Ask the agent to "create a lookup `X` with these rows" and it just works; column types are inferred from the values you send (numbers as numbers, ISO date strings for datetimes).
- **`add_lookup_data` now sends each row in the shape the API expects.** It previously sent the whole batch as a JSON array under `data`, which the Cortex endpoint rejects (it expects one row object per write) — the root cause of the hang. The connector now writes each row as its own object, looping for multi-row batches, and reports `{ dataset_name, rows_written, results }`. `key_fields` still make the write an upsert. Creating/populating a lookup remains an approval-gated write action.
