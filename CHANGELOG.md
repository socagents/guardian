...
- **Guardian can now install on RHEL with Podman and no Docker at all.** A new `guardian-installer-podman` targets RHEL 8 + rootful Podman: it installs `podman` + `podman-docker`, enables the rootful Podman API socket (Docker-compatible, at `/var/run/docker.sock`), uses a Podman compose variant (the updater runs `security_opt: [label=disable]` so its socket mount works under SELinux-enforcing RHEL), and otherwise follows the exact same install ceremony as the Docker installer. The existing Docker installer and Docker customers are unchanged (the build is one runtime-switched source; `RUNTIME` defaults to docker).
- **The updater works through Podman's Docker-compatible socket.** Image pulls now pass explicit registry credentials (`auth_config`) and the Docker SDK client honors `DOCKER_HOST` + pins API-version negotiation — so connector pulls and stack updates authenticate correctly against Podman's compat endpoint (and remain a no-op on Docker).
...
- **Beta:** the Podman path is documentation- and unit-validated but certified on the first customer RHEL/Podman install (no Podman host in CI). The auth-on-pull, dynamic connector spawn, and agent→connector DNS are the items to confirm there.
