...
- *Verified end-to-end on a fresh RHEL 8.10 + Podman VM with outbound egress default-DROP except the one download host: OS auto-detected as tested, runtime auto-detected as Podman, every image loaded from the bundle, agent + updater healthy, UI reachable — with `ghcr.io` provably blocked (nothing pulled from a registry).*
...
- **Corrects an over-broad caveat about `event_sub_type`.** v0.2.96 told the agent not to filter on `event_sub_type` at all — but that's wrong: **`filter event_type = ENUM.PROCESS and event_sub_type = ENUM.PROCESS_START` works** (and usefully narrows to true process starts); only **`comp ... by event_sub_type`** fails ("the event_sub_type field cannot be queried…"). The `cortex_xql_query_authoring` and `cortex_detection_rule_authoring` skills and the affected KB entry now state the precise rule: filter on it freely, never use it as an aggregation key. Found by re-verifying the 102 queries (several use that filter).
...
- **The agent now verifies an XQL query before handing it to you.** A new **`xsiam_xql_verify`** tool runs an authored query on a narrow window first and returns a verdict — does it parse, what columns + sample rows come back, and the compute-unit cost — so a *silently-wrong* query (the dangerous kind: HTTP 200 with plausible-but-incorrect rows, like an `incidr` test that quietly matches nothing) is caught before an analyst acts on it. The authoring skill now treats "verify, check the sample values, then return" as a required step.
- **44 new live-verified XQL examples in the knowledge base** (now 281 entries), every one run on a real tenant before it was saved, each tagged with its measured CU cost: real-telemetry IR hunts (process trees, beaconing, registry persistence, external-egress exfil, LSASS access), the full stage/function coverage matrix (windowcomp, joins, time math, arrays, string functions), ATT&CK-tagged impact/defense-evasion hunts (ransomware, inhibit-recovery, disable-Defender, LOLBins), and field-discovery idioms.
- **The agent knows the names that silently fail.** XQL reports an unknown function as a bare HTTP 500 with no hint, and one bad token poisons the whole query — so an agent reaching for Splunk/KQL habits (`dcount`, `stddev`, `percentile`, `case`, `lead`) used to fail mysteriously. The authoring skill now carries the verified vocabulary *and* the substitution table (`count_distinct`, `sqrt(var())`, nested `if`, `lag`-only), so those failures don't happen.
- **Airtight field discovery — the agent stops guessing field names.** `xsiam_datamodel_describe` was fixed (it relied on a `datamodel` stage that isn't available on every tenant, and mis-read the query id) and now falls back to sampling real rows, so it always returns the dataset's actual fields. Paired with `xsiam_get_datasets`, the agent can author across the **hundreds of datasets** a tenant ingests (cloud audit logs, firewall events, forensic artifacts), not just endpoint telemetry — discovering each one's schema before writing the query.
- **Authoring a Cortex detection now has a skill.** A new **`cortex_detection_rule_authoring`** skill covers writing scheduled **correlation rules**: the query-vs-rule-wrapper boundary, the `filter → bin → comp → threshold → project-entities` shape, mapping output columns to alert fields, suppression and the 5000-issue/24h auto-disable ceiling, and severity→case semantics. It ships with four correlation-rule XQL bodies (failed-logon brute force, Office→shell, beaconing, ransomware) whose queries all run clean on a real tenant.
- **CU-aware planning.** Authoring now extrapolates a wide hunt's cost from the cheap verification run and checks it against your daily quota *before* spending it — narrowing the window or warning you with the number rather than silently blowing the budget.
- **A regression harness** (`scripts/xql_regression/`) keeps this knowledge honest: a 24-case corpus that re-checks the known-good idioms *and* asserts the known-bad names still fail, so if Cortex ever changes XQL semantics, drift is caught.
...
- **The updater works through Podman's Docker-compatible socket.** Image pulls now pass explicit registry credentials (`auth_config`), the Docker SDK client honors `DOCKER_HOST` + pins API-version negotiation, and the streaming pull falls back to a non-streaming pull if Podman's compat socket rejects the Moby streaming protocol — so connector pulls, stack updates, and in-app "Update now" complete correctly against Podman's compat endpoint (and remain no-ops on Docker).
- **Delivered through the registry too.** Both installers publish as GHCR images (`ghcr.io/kite-production/guardian-installer` and `…-podman`), so customers whose egress allow-lists permit `ghcr.io` but not `github.com` can obtain the installer with `podman pull` + `podman run … cat /guardian-installer`.
- **Beta — to be certified on the first customer RHEL/Podman install** (no Podman host in CI; the dev cycle generation-validates the binary via `bash -n` + marker checks each push). Confirm on the customer box: authenticated `ghcr.io` pull through the compat socket, dynamic connector spawn, agent→connector DNS, and reboot survival. A pre-tag adversarial review of the diff caught and fixed two would-be first-install bricks (EPEL-only compose provider; missing reboot unit) before this tag.

## [v0.2.92] (2026-06-28) — *CU forecasting skill: best-practices playbook for saving Compute Units (#94)*

- **The `cortex_compute_unit_forecasting` skill now carries an 18-practice playbook for cutting CU spend**, replacing the earlier short levers list. It's grouped so the agent can apply it end-to-end: **(A) author cheaper queries** — shrink the time window (and, for Cold Storage, don't straddle a UTC midnight you don't need, since cold rounds up to whole days); filter early on real fields (confirm them with `xsiam_datamodel_describe`); `| limit` is *not* a cost lever; aggregate server-side to return a rollup not thousands of rows; answer multiple questions in one multi-aggregation pass; don't run per-row enrichment (`iploc`) or wide presets over the firehose — filter first then enrich, or push enrichment into a near-free lookup. **(B) route hot vs cold** — default to hot, and for a deep cold dive exploit the free 7-day rewarm cache (pay once, iterate free on a fixed range), narrowing on hot first then cold-sweeping filtered. **(C) operate within budget** — calibrate with `compute_units_used` before scaling, pre-flight with `xsiam_get_xql_quota` (0 CU), skip XQL when the fact is already on the incident/alert/endpoint object, respect the ~5-concurrent cap (parallel API keys don't buy more budget), tune the daily limit + spread work across the 00:00-UTC reset, and monitor/self-throttle. Each practice was adversarially reviewed against the documented cost model — no invented constants.

## [v0.2.91] (2026-06-28) — *XQL Compute-Unit forecasting: see + budget your query quota (#93)*

- **The agent can now tell you your Cortex Compute-Unit (CU) quota and what each query costs.** XSIAM meters data-lake queries in Compute Units against a configurable **daily limit**; when it's spent, queries are rejected. A new **`xsiam_get_xql_quota`** tool reads the live quota *without running a query* (it consumes no CU) — annual bank (license + purchased + eval), cumulative `used_quota`, computed `remaining_annual_cu`, today's `daily_used_quota`, and the daily running-query + concurrency counters. Ask the agent "what's my XQL quota?" or "how much headroom do I have today?" before a wide hunt.
- **Every XQL query now reports its own cost.** `xsiam_run_xql_query` surfaces `compute_units_used` (the CU this query cost), `remaining_quota_cu` (CU left), and `number_of_results` alongside the rows — Cortex already returns these in the query result; Guardian now labels them. And a `QUOTA_EXCEEDED` rejection returns a clear message: the daily cap is reached (resets 00:00 UTC), with a hint to narrow the time window, filter early, or target a lookup dataset.
- **A new `cortex_compute_unit_forecasting` skill** teaches the whole model: what consumes CU (XQL API / Cold Storage / Notebooks / BQ), the accounting (free annual quota by license, the +1-CU/day add-on, daily-limit options, no rollover, 00:00-UTC reset), an empirical cost model calibrated on thousands of real queries (**cost scales with data scanned, not query complexity** — lookups are ~free, wide `xdr_data`/`iploc`/`metrics_view` scans are where CU goes), how to forecast queries-per-budget, and the levers to cut spend (narrow the window — the dominant lever — filter early, prefer lookups, avoid wide per-row enrichment; `| limit` does *not* reduce scan cost). It also covers the **two cost regimes** — hot/standard queries (calibrate: there's no published CU-per-GB constant, so probe + read `query_cost`) vs **Cold Storage (analytical: 1 CU per 35 GB queried, time windows rounded up to whole days, re-queries on the same range free within a 7-day rewarm)** — and ships **5 worked customer estimation examples**: IR investigation, threat hunting via API, compliance reporting (cold), SOC automation, and DFIR retrospective breach scoping (cold), each with a profile → workload → CU math → verdict → recommendation.
- **No public API exists** for the per-query usage *list* (UI-only, Settings → Configurations → Data Management → Compute Unit Usage) or for *setting* the daily limit / enabling the add-on (UI-only) — so capture per-query cost from each query response, and read aggregate state from `xsiam_get_xql_quota`.

## [v0.2.90] (2026-06-28) — *XQL authoring perfected from a 20-scenario live program (#92)*

- **The XQL-authoring skill is now a real first-shot reference, hardened against a live tenant.** We ran a 20-scenario program — each scenario builds a realistic dataset (process telemetry, beaconing, IOC feeds, brute-force logs, lateral movement, exfil flows, JSON payloads, delimited fields, regex logs, CIDR lists, geo-IP) and authored 12+ complex XQL queries against it, run for real against a live XSIAM tenant. **187 queries executed live; every failure became a documented skill fix.** The `cortex_xql_query_authoring` skill now carries verified gotchas that previously caused silent failures: `bin` is a standalone stage (not a function); no infix arithmetic — use `divide()/multiply()/add()/subtract()`; no SQL `OVER()` — window analytics use the `windowcomp` stage; after a `join … as a`, reference the joined dataset's columns downstream by **bare** name (not `a.col`); `json_extract_scalar` returns strings (wrap `to_integer` for math) and `json_extract_array` quote-wraps elements (use `json_extract_scalar_array` to explode-then-filter); `regexcapture()` silently no-ops in queries (use one `regextract` per group); the `iploc` geo-enrichment stage; IP/CIDR helpers (`incidr`, `incidrlist`, `is_known_private_*`); and `object_create`/`values()`-vs-`list()` for building JSON.
- **36 new live-verified example queries in the `xql-examples` knowledge base (201 → 237).** Each is a complex, multi-stage IR/threat-hunting query that ran successfully against a real tenant — beacon coefficient-of-variation scoring, RDP lateral-movement fan-out with crown-jewel verdicts, JSON-payload risk geography, brute-force-vs-compromise heuristics, fleet-relative exfil ranking, regex log extraction, CIDR zone classification, geo-IP aggregation, and more. They're embedded and searchable via the agent's `xql_examples_search`, so the agent retrieves a known-good pattern before authoring.
- **The bundled `dataset_fields.md` reference now lists the flat `xdr_data` fields** (process/network/file/registry/auth) used by direct `dataset = xdr_data` queries — previously it carried only the `xdm.*` datamodel paths, leaving the agent to guess flat field names.
...
## [v0.2.88] (2026-06-27) — *XSIAM lookup datasets: create + populate, with Advanced-auth keys (#91)*

- **Creating + populating an XSIAM lookup dataset from chat now works end-to-end.** The two-step flow is **`create_dataset`** (POST `xql/add_dataset` with a typed `{column: type}` schema) → **`add_lookup_data`** (populate the now-existing dataset). The earlier "hang" had a single root cause uncovered by validating every endpoint with raw `curl`: **this tenant's API keys require Advanced (signed) auth** — each request signed with a SHA-256 nonce + timestamp — and the instance was using a Standard key, which the write endpoints reject by stalling. With an Advanced key (set the connector's **API key security level** to *Advanced*), `add_dataset`, `add_data`, `get_data`, and XQL read-back all succeed.
- **`add_lookup_data` sends `data` as an array in one request.** The Cortex endpoint wants `data` as a **list of row objects** `[{…}, {…}]` — a bare object makes it silently *skip* every row. The connector sends the whole batch in a single request (the endpoint doesn't support concurrent edits), accepts a lone dict as a one-row list, and returns `{ rows added, rows updated, rows skipped }`. `key_fields` make the write an upsert.
- **The dataset must exist before populating it.** `add_lookup_data` no longer claims to auto-create; create the lookup first with `create_dataset`. Both are approval-gated writes; `remove_lookup_data` stays manifest-denied.
- **"Test Connection" now works for Advanced keys.** The XSIAM instance health probe (Test Connection / the status dot) only ever did Standard auth, so it returned `Probe failed: HTTP 401 (creds rejected)` for a perfectly valid **Advanced** key — even though the agent's tool calls authenticated fine. The probe now signs the request with the same nonce + timestamp scheme as the connector when the instance's **API key security level** is *Advanced*.
