...
export const RELEASE_NOTES: ReleaseNote[] = [
  ...
  },
  {
    version: "0.17.128",
    date: "2026-06-07",
    title: "guardian-updater reconciles connector containers to their pins on a timer, not just at startup.",
    highlights: [
      "Per-instance connector containers now track their pinned image digest on a periodic loop (default 5 min; GUARDIAN_UPDATER_RECONCILE_INTERVAL_S), not only at updater startup.",
      ...
      "Same reconcile path the startup + manual endpoints use — compares each guardian-connector-* container's digest to its pin and recreates only the divergent ones, sequentially.",
    ],
  },
  {
    version: "0.17.127",
    ...
    highlights: [
      ...
      "Simulation confirmations now name the worker they started: the no-text fallback recap previously omitted guardian_create_data_worker (a 'simulate Okta to XSIAM' turn recapped only an incidental memory_store). It now counts simulation + catalog/data-source mutations.",
    ],
  },
  {
    version: "0.17.126",
  ...
  },
  {
    ...
    highlights: [
      ...
      "guardian_create_data_worker's vendor/product arg docs now spell out the literal-not-display rule + the Okta trap, so the agent gets routing right at the call site.",
  ...
  },
  {
    ...
    highlights: [
      "Fixed guardian_kill_worker, which returned a 'request is not defined' error on every call (a v0.17.114 flatten regression) — the worker did stop, but agent-driven cleanup saw a failure and could leave workers emitting.",
  ...
  },
  {
    ...
    highlights: [
      "Mint a scoped, revocable API key in /api-keys and send Authorization: Bearer guardian_ak_… to /api/chat + /api/agent/*.",
  ...
  },
  {
    ...
    date: "2026-05-28",
    title:
      "xlog hot-fix: guardian_kill_worker flattens its signature — same bug-family as v0.17.76's guardian_create_data_worker.",
    highlights: [
      "Live-discovered during chat-driven ServiceNow smoke (session 8b67819e): agent ran the full discover→fire→verify cycle, then guardian_kill_worker(worker_id=...) failed with 'Missing required argument: request' / 'Unexpected keyword argument: worker_id'. Worker leaked indefinitely.",
      ...
      "Fix: guardian_kill_worker now takes worker_id: str directly (mirrors v0.17.77's create_data_worker flatten). KillWorkerRequest Pydantic class stays defined for internal use; no agent path traverses it now.",
      "Bug-family audit: guardian_create_data_worker (already flat v0.17.77 ✓), guardian_list_workers (no args ✓), guardian_kill_worker (flat now ✓), guardian_create_scenario_worker still has its envelope — tracked for the next time a scenario smoke surfaces it.",
    ],
  },
  {
    version: "0.17.91",
  ...
  },
  {
    ...
    highlights: [
      "Operator-surfaced after Gemini 3.5 Flash smoke: the model's extended-thinking text streamed lumped with the final reply (\"My Thought Process as Guardian MCP Agent...\" before the actual answer to \"hi\").",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Upstream behavior: @anthropic-ai/claude-code refuses to start with --permission-mode bypassPermissions when process.getuid()===0. The guard's premise ('we can't tell if you're sandboxed') is false for guardian_agent — it IS a sandbox by inspection (Docker, bind-mounts only, ephemeral FS).",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Guardian stays destination-neutral — the schema doesn't gain XSIAM-specific fields; the routing guidance lives in operator-facing how_to_use markdown that the drawer's 'How to simulate' section renders (already wired in v0.17.75).",
  ...
  },
  {
    ...
    date: "2026-05-28",
    title:
      "xlog: flatten guardian_create_data_worker signature — agent's chat path now reaches the connector end-to-end.",
    highlights: [
      "Sequel to v0.17.76. The agent's MCP proxy is fixed at the connector.yaml boundary, but the connector container's Python tool still rejected the flat args because its signature was `guardian_create_data_worker(request: CreateDataWorkerRequest, ctx: Context)` — FastMCP advertised a single `request` parameter, not the individual fields.",
      "Refactored: guardian_create_data_worker now takes individual keyword args (type, *, destination, count, interval, vendor, product, schema_override, etc., ctx). Function body keeps using request.X accessors by reconstructing CreateDataWorkerRequest inside.",
  ...
  },
  {
    ...
    date: "2026-05-28",
    title:
      "xlog: fix guardian_create_data_worker arg schema — the agent's proxy now accepts vendor-faithful schema_override end-to-end.",
    highlights: [
      "Bug: agent's guardian_create_data_worker proxy validated args against a stale connector.yaml shape (format, rate_per_second, duration_seconds, observables, destination-as-object) — every meaningful field (type, count, vendor, product, schema_override) was rejected with 'Unexpected keyword argument'.",
      ...
      "Surfaced via v0.17.75 MCP-tool smoke pass on guardian-vm — the agent's chat path can now actually drive vendor-faithful streaming via the data_sources_get_schema → guardian_create_data_worker(schema_override=...) chain.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "New cli-wrapper.ts feature: clearEnv field deletes keys from the inherited env after the envVars overlay — needed because /opt/guardian/.env may have a stale ANTHROPIC_API_KEY that would otherwise shadow the OAuth token.",
    ],
  },
  {
    version: "0.17.71",
  ...
  },
  {
    ...
    highlights: [
      ...
      "New `simulate-from-installed-data-source` journey (category: simulation, intermediate, 2 min) — focused on log generation: data_sources_list → data_sources_get_schema → guardian_generate_fake_data_v2 (or guardian_create_data_worker for UDP destinations). Two prompts covering both in-chat preview and UDP-destination variants.",
  ...
  },
  {
    ...
    highlights: [
      "Every description follows the same 2-sentence shape: <vendor concept>. <wire-shape constraint per type>. — teaches Guardian what form the value must take so modeling rules (Cortex, Splunk, Elastic) succeed.",
  ...
  },
  {
    ...
    highlights: [
      "New 'Before you start' callout on each journey detail page lists prerequisite journeys as clickable cards. Guidance-only — Guardian doesn't block execution, but operators landing cold see what setup is missing.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "v0.17.54's /api/chat/cli now resolves credentials from the store directly; ANTHROPIC_API_KEY in /opt/guardian/.env stays as a backup path but is no longer required.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Claude Code is baked into the guardian-agent image (@anthropic-ai/claude-code@^1.0.0), so first invocation is sub-second instead of waiting for an npx download.",
      "New lib/cli-wrapper.ts ports Spark's plugin-runner pattern to Guardian's single-container model — child_process.spawn instead of Docker-in-Docker.",
  ...
  },
  {
    ...
    date: "2026-05-25",
    title:
      "Help docs: integration-neutral language pass (round 1) — Guardian is destination-neutral, not an XSIAM accessory.",
    highlights: [
  ...
  },
  {
    ...
    highlights: [
      ...
      "Architecture page rest-api section: 'HTTP Basic auth on the agent side' claim corrected to 'guardian_session cookie' in two places.",
  ...
  },
  {
    ...
    highlights: [
      "Architecture page: added 7 sections to the left rail nav (image-pinning, tls-proxy, guardian-updater, connector-containers, data-sources, log-destinations, operator-state). They rendered before but were invisible to the sidebar.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Retired the 'Upgrade Guardian from v0.2.x → v0.3.0' journey. No current customer is on v0.2.x.",
  ...
  },
  {
    ...
    date: "2026-05-25",
    title:
      "Architecture page (/help/architecture) rewritten to describe Guardian as it is today, not as it evolved — release annotations and change-history framing stripped.",
    highlights: [
  ...
  },
  {
    ...
    date: "2026-05-25",
    title:
      "Login page polish — \"Guardian can …\" capabilities cycle + Guardian logo in the bottom orb row.",
    highlights: [
      "FlippingText cycle re-framed: was \"Powered by [Cortex XSIAM, MITRE ATT&CK, CALDERA, Vertex AI, Gemini, MCP]\"; now \"Guardian can [simulate logs, simulate attacks, create workers, send logs, validate detections, orchestrate workflows]\". Same 6-item count, same cyan typewriter animation, same cursor blink — only the prefix string + words array changed.",
      "Why the change: the brand-name cycle suggested Guardian was a thin shim over third-party products; the capability cycle tells the operator what Guardian actually does for them.",
      "ToolsStackCard bottom-of-page row: the middle (largest) orb's gold bolt glyph is replaced with the actual Guardian logo (/logo.svg, same asset as the sidebar). 4 outer orbs (smart_toy, memory, monitoring, api) keep their existing glyphs + colors.",
  ...
  },
  {
    ...
    date: "2026-05-24",
    title:
      "Docs: bake XSIAM dataset-routing conventions into handler docstrings + spec yamls + help docs + xlog agent prompt. xsiam_http always → guardian_logs_raw; syslog+CEF+broker → <vendor>_<product>_raw. Verified live during operator hands-on smoke; baked in so the agent no longer guesses dataset names when verifying ingestion.",
    highlights: [
      "xsiam_http handler + spec.yaml: documents guardian_logs_raw as the always-landing dataset + canonical XQL verify template.",
      "syslog handler + spec.yaml: documents <vendor>_<product>_raw broker-CEF convention; points at guardian_create_data_worker(type=CEF, vendor=..., product=...) as the production path.",
      "xlog connector's guardian_create_data_worker docstring: teaches the agent the dataset-routing table + ready-to-run XQL verify templates so 'verify the records arrived' becomes one tool call, not a guessing game.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "guardian_create_data_worker docstring rewritten: agent now calls log_destinations_get(name), formats per type (syslog → udp:host:port; xsiam_http → XSIAM_WEBHOOK), then passes the string to xlog.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Secret slots persisted to SecretStore at /agents/guardian/log_destinations/<id>/<slot>; cascade-deleted on row delete; '***' sentinel preserves on PATCH.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "scripts/e2e_v0161_multi_instance_and_fields.py — schema lookup nested under data_source key. 23/23 PASS on guardian-vm.",
  ...
  },
  {
    ...
    highlights: [
      "guardian_create_data_worker MCP tool accepts optional schema_override (the fields[] array from data_sources_get_schema). When supplied with a udp:/tcp: destination, the worker streams vendor-faithful records.",
      ...
      "New MCP tool data_sources_installed_as_vendors returns installed sources as tech-stack-shaped vendor entries (vendor, product, category, source='installed'). Use AND with guardian_get_technology_stack — agent merges in reasoning.",
  ...
  },
  {
    ...
    highlights: [
      "/jobs page YAML-issues banner works again — fetchYamlIssues was sending guardian_auth=${token} server-side; v0.9.1's middleware rejected it. One-line fix to guardian_session=${token}.",
      "6 doc-drift updates — mcp/agent/CLAUDE.md, docs/CICD.md (2 spots), settings/backup-restore JSDoc, profile/page.tsx JSDoc, and 2 journey descriptions now correctly reference guardian_session + the middleware.ts (v0.9.1+) gate.",
      ...
      "Historical context preserved: CHANGELOG entries, past-tense comments, and the v0.4.0 + v0.9.1 release notes correctly keep their guardian_auth references — they describe the pre-rename state and are immutable historical record.",
    ],
  },
  {
    version: "0.9.1",
    ...
    security: true,
    title:
      "Security fix — server-side session cookie enforcement on every /api/agent/* + /api/chat + /api/skills/* endpoint. Pre-v0.9.1 AuthGate gated the BROWSER but not the API; any caller that reached port 3000 could control the agent with NO cookies. New Next.js middleware validates guardian_session against the MCP-side session store on every request. Companion fix: Backup/Restore have been completely broken since v0.4.0 (hardcoded the pre-v0.4.0 cookie name) — now work for the first time on the v0.4.x+ cookie surface.",
    highlights: [
      "Closes #70 — every /api/agent/* mutation endpoint, /api/chat, and /api/skills/* now requires a valid guardian_session cookie. Absence or invalid value → 401.",
      ...
      "Backup and Restore work for the first time since v0.4.0. Pre-v0.9.1 both routes hardcoded the pre-v0.4.0 cookie name (guardian_auth) and always returned 401. Discovered during the explorer-subagent map of the auth surface.",
      ...
      "Architecture page #authentication updated to reflect the middleware layer and the corrected guardian_session cookie name throughout (was still saying guardian_auth in places).",
    ],
  },
  {
    version: "0.9.0",
    ...
    highlights: [
      ...
      "All operator-visible surfaces (UI copy, tool docstrings, marketplace card, release notes) describe the catalog as just shipping with Guardian.",
  ...
  },
  {
    ...
    date: "2026-05-21",
    title:
      "Marketplace Data Sources polish — catalog now reads instantly from local files; logos serve through Guardian. Same operator-visible feature set as v0.8.0, with the runtime path simplified to remove all references to remote sources. Path traversal defense + 18 unit tests pin the safety contract.",
    highlights: [
      "Catalog endpoint serves the rolled-up packs/rules from the local bundle. Sub-millisecond response.",
      "Logo route GET /api/agent/data-sources/logo/{pack} streams SVG/PNG bytes with Cache-Control immutable. Logos always render through Guardian — no third-party fetches.",
  ...
  },
  {
    ...
    date: "2026-05-21",
    title:
      "Marketplace Data Sources catalog ships with Guardian. Browse loads instantly, install is local, logos serve through Guardian. Catalog includes 197 packs / 232 schemas / 145 logos (2.9 MB bundled).",
    highlights: [
  ...
  },
  {
    ...
    highlights: [
      "End-state acceptance check GREEN on guardian-vm dev-9f1a576: install FortiGate via REST → 176 fields persisted → xlog generateFakeDataV2 with the schema override emits {srcip, dstip, srcport, dstport, proto, action, user, sentbyte, rcvdbyte} — FortiGate's actual field names with plausible values. schemaApplied: true, fallbackReason: null.",
  ...
  },
  {
    ...
    date: "2026-05-21",
    title:
      "Phase 4 of the v0.8.0 arc — the agent learns to use installed Data Sources. New simulate_vendor_logs skill triggers when the operator names a specific vendor (\"simulate FortiGate logs\"). The skill chains data_sources_list → data_sources_get_schema → xlog generate_fake_data_v2 with schema_override. New guardian_generate_fake_data_v2 MCP tool exposes the v0.7.10 GraphQL field to the agent.",
    highlights: [
      "guardian_generate_fake_data_v2 MCP tool wraps xlog's generateFakeDataV2 GraphQL. Same FakeDataRequest as v1 + optional schema_override (vendor_fields list extracted from a Data Source). Backward-compat: no override → identical to v1.",
      ...
      "New skill in bundles/spark/mcp/skills/workflows/simulate_vendor_logs.md (165 lines). Triggers on phrases like 'simulate FortiGate', 'generate PaloAlto events', 'create Okta audit records'. Falls back to the legacy guardian_generate_fake_data path when the request is generic (no vendor name).",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Installed grid: 1/2/3-column responsive cards. Real vendor logos served through Guardian when present (FortiGate SVG renders inline). Status chip distinguishes structured ('N vendor fields') from rawlog-only packs.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Mid-arc commit per CLAUDE.md release-readiness gate: NO customer release tag fires for v0.7.5 — the v0.8.0 tag waits for arc completion (Phase 4). Build + auto-deploy on guardian-vm run automatically; smoke runs against the auto-deployed install.",
    ],
  },
  {
    version: "0.7.4",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Settings/personality: dropped \"Settings / Personality\" breadcrumb (no other settings/* page had one), swapped GuardianLogo box for the standard psychology material icon.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Filed: #68 (guardian-updater missing-container auto-spawn) · #69 (journeys API paths — fixed inline) · stale count cleanups (4→5 services, 3→7 connectors, 11→20 skills, 161→787 KB entries across system-prompt + 4 help files).",
    ],
  },
  {
    version: "0.7.0",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Lifespan startup schedules the reconcile as a background task ~30s after guardian-updater starts (delay covers agent boot, since the recreate calls back to the agent for container_url). Fire-and-forget — /healthz stays responsive, docker-compose healthcheck doesn't cascade-fail.",
      ...
      "Picked the guardian-updater-hook layer (not workflow patch, not installer patch) because guardian-updater IS the component responsible for connector lifecycle. It restarts on every install (docker-compose recreates it when its image digest changes), so the new hook fires automatically on every install — customer AND dev.",
  ...
  },
  {
    ...
    date: "2026-05-20",
    title:
      "Connector instance API responses now include container_url. Field exists in the database + dataclass + is set correctly by guardian-updater's callback — but three serializers were silently dropping it from API responses (HTTP /api/v1/instances + instances_list + instances_get MCP tools).",
    highlights: [
      ...
      "Write path was always correct: guardian-updater's _agent_set_container_url callback POSTs the URL → PUT handler → SQLite UPDATE all worked. Only the read paths were broken.",
  ...
  },
  {
    ...
    date: "2026-05-19",
    title:
      "Removed stale guardian-mcp:8080 fallback URL from mcp-proxy.ts. The guardian-mcp service was collapsed into guardian-agent in v0.1.30; the fallback pointed at a service retired ~5 releases ago.",
    highlights: [
      "Pre-v0.1.30 there was a separate guardian-mcp container in the compose network. v0.1.30 collapsed it into a subprocess inside guardian-agent on localhost:8080. The fallback URL in mcp/agent/lib/mcp-proxy.ts:36 stayed pointing at the old hostname.",
      "Production wasn't affected because installer/docker-compose.yml always sets MCP_URL=http://localhost:8080/... in the guardian-agent env block. But if MCP_URL were ever unset for any reason (manual operator edit, runtime-config refresh hiccup), the fallback would have produced EAI_AGAIN: guardian-mcp DNS failures.",
      ...
      "Bug-family audit: grep -rn 'guardian-mcp' mcp/agent/ bundles/spark/ — remaining hits are observability-probe URLs in bundles/guardian-agent.bundle.yaml + bundles/observability.contract.yaml (not load-bearing; out of scope for this contained release), plus historical release-notes/journeys references intentionally preserved.",
  ...
  },
  {
    ...
    date: "2026-05-19",
    title:
      "guardian-updater now handles space-bearing instance names (Cortex XDR, Cortex Docs Search). Pre-v0.6.43 the reconcile/start/stop/restart endpoints rejected these as 'invalid path segment' — so existing instances couldn't be re-image-pulled after a connector code change.",
    highlights: [
  ...
  },
  {
    ...
    highlights: [
      ...
      "Fix: new deriveConnectorId(toolName) helper at module scope. Dotted form (cortex-xdr.get_alerts) splits on '.' as before. Flat aliases use an explicit longest-prefix mapping table: guardian_web_→web, guardian_→xlog, xdr_→cortex-xdr, caldera_→caldera, xsiam_→xsiam. cortex_ disambiguated by tool-name lookup (small fixed set of 6 cortex-docs tools; everything else cortex-content). Built-in MCP tools (instances_list, audit_recent, jobs_*, etc.) return null — caller skips connector state update.",
  ...
  },
  {
    ...
    highlights: [
      "Verification of v0.6.37's deployed code revealed the miss: grep 'log.search' inside the running guardian_agent container returned 3 hits, only 1 of which was the v0.6.37 fix. audit_search and metrics_health were still broken.",
  ...
  },
  {
    ...
    date: "2026-05-19",
    title:
      "Connector containers no longer show as (unhealthy). The HEALTHCHECK CMD in guardian-connector-runtime was calling wget, which doesn't exist in python:3.12-slim's Debian trixie base — every connector container had been reporting unhealthy since v0.5.x with thousands-deep failing-streak counters.",
    highlights: [
      ...
      "Why this didn't block anything operationally: guardian-updater routes MCP calls on Container.Up, not Container.Health. The (unhealthy) label was purely cosmetic — every connector tool call succeeded normally. Confused operators, didn't break anything.",
      "Fix: replace wget with python3 -c urllib.request.urlopen. Python is already in the container (it IS a Python container), zero dependency change. Same endpoint, same semantics, just a tool that exists in the image. The guardian-browser/Dockerfile literally has a comment explaining why it skipped HEALTHCHECK ('minimal image without wget, curl, sh, python, or even nc') — the connector-runtime should have read that comment before assuming wget was available.",
      "Bug-family audit per CLAUDE.md §7: grep -rn HEALTHCHECK across the repo. Only guardian-connector-runtime/Dockerfile had the wget bug. All other services either don't define HEALTHCHECK or document why they can't.",
  ...
  },
  {
    ...
    date: "2026-05-19",
    title:
      "audit_recent MCP tool now works. Pre-v0.6.37 every call raised AttributeError 'SqliteAuditLog object has no attribute search' — implementation called guardian methods (recent, search) that don't exist. Canonical method is query().",
    highlights: [
      ...
      "Smoke 1 (MCP-side): audit_recent(limit=5) from inside the guardian_agent container returns 5 most-recent audit rows. Smoke 2 (chat-side): ask the agent 'show me the 10 most recent audit events' — post-fix the tool result is actual rows instead of an AttributeError.",
    ],
  },
  {
    version: "0.6.36",
    date: "2026-05-19",
    title:
      "GUARDIAN_CHAT_MAX_TURNS env var is now actually operator-tunable — added bare-name forwarding to both compose files. v0.6.32's release notes promised this but the plumbing was an audit-miss; setting the var in .env had no effect.",
    highlights: [
      "Discovered during v0.6.35 smoke testing — tried to set GUARDIAN_CHAT_MAX_TURNS=2 to deterministically reproduce the dropped-final-response bug; the container kept using the compiled default of 30. The chat-route code reads process.env.GUARDIAN_CHAT_MAX_TURNS at line 4190, but docker-compose only passes through env vars explicitly listed in each service's environment: block.",
      "Fix is two lines (one per compose file): - GUARDIAN_CHAT_MAX_TURNS added to guardian-agent service's environment block in installer/docker-compose.yml and the dev docker-compose.yml. Bare-name forwarding lets the operator set the var in .env; if unset, the route.ts default-30 path triggers naturally.",
      ...
      "Smoke 1 (no regression): with GUARDIAN_CHAT_MAX_TURNS unset, route uses default 30. Smoke 2 (the actual fix): set =2 in .env, restart guardian_agent, run a 3+ tool query. logDebug 'Recovered N chars of post-budget text from final unprocessed response' visible in docker logs guardian_agent confirms v0.6.35 + v0.6.36 working together.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Deterministic repro for smoke: set GUARDIAN_CHAT_MAX_TURNS=2 in the agent container, then run a chat query that needs 3+ tool calls plus a summary. Pre-fix: summary text is lost. Post-fix: summary text appears in the chat bubble as text_delta.",
    ],
  },
  {
    version: "0.6.34",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Primary chat handler's hardcoded `for step < 20` replaced with MAX_AGENT_TURNS env-driven cap (default 30, clamped to [5, 200]). Operators can tune via GUARDIAN_CHAT_MAX_TURNS.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Interactive flow unchanged: 'docker exec -it guardian_agent node /app/cli/reset-admin.mjs' still works exactly as before. Non-interactive is automation-only.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Lesson on contained-release smoke: ALWAYS run the post-deploy smoke before claiming a fix complete. v0.6.22's local tsc+lint+build green hid the runtime env-override gap. Real verification requires hitting the actual endpoint on guardian-vm.",
    ],
  },
  {
    version: "0.6.23",
  ...
  },
  {
    ...
    highlights: [
      "xlog's _resolve_ssl_args() priority 1 is the shared guardian_tls volume — xlog mounts /tls/cert.pem (from guardian-agent's auto-generated cert) and serves HTTPS unconditionally. The architectural truth has been HTTPS-since-v0.4.0; the code defaults never caught up.",
  ...
  },
  {
    ...
    date: "2026-05-19",
    title:
      "guardian-connector-cortex-xdr was missing from the release manifest AND the dev-installer digest loop since v0.5.61. Surfaced when an operator restart of cortex-xdr fell to tag-pinning and tried to pull `:dev-<sha>` — a tag that only exists when build-connectors fires for that commit.",
    highlights: [
      ...
      "Every customer release manifest since v0.5.61 has shipped without DIGEST_GUARDIAN_CONNECTOR_CORTEX_XDR. Every dev manifest since v0.5.61 has shipped without it too.",
  ...
  },
  {
    ...
    date: "2026-05-19",
    title:
      "Bug-family audit completion for the v0.6.17 KEK fix — GUARDIAN_AGENT_INTERNAL_URL + GUARDIAN_TLS_VERIFY were reading from os.environ too, silently ignoring operator .env overrides.",
    highlights: [
      "v0.6.17 fixed GUARDIAN_SECRET_KEK by switching to _host_env_get() (reads /host/.env directly). CLAUDE.md § 'Bug-family audit (v0.5.80)' mandates that sibling instances of the same pattern get audited in the same release-discipline window.",
      "Audit found two more hits: _agent_tls_verify() and _resolve_agent_internal_url() both read GUARDIAN_AGENT_INTERNAL_URL / GUARDIAN_TLS_VERIFY from os.environ. The compose guardian-updater.environment block intentionally doesn't pass them through (v0.5.51 env-block stability invariant), so any operator .env override was a silent no-op.",
      ...
      "Three call sites swapped to _host_env_get(). Post-audit grep returns zero hits for the bug-family. GUARDIAN_REGISTRY* env reads remain os.environ.get() intentionally (passed through compose for docker login at boot) — documented in CHANGELOG.",
    ],
  },
  {
    version: "0.6.17",
    date: "2026-05-19",
    title:
      "guardian-updater was passing EMPTY GUARDIAN_SECRET_KEK to spawned connector containers since v0.5.51, so all instance secrets resolved to empty strings. Layer-6 of the kill-chain unblock chain; fixes 'no apiKey configured' error after v0.6.16.",
    highlights: [
      ...
      "Host KEK length: 43 (real). Container KEK length: 0 (empty). Connector-runtime log: 'GUARDIAN_SECRET_KEK is not set on this connector container.'",
      "Root cause: updater/src/main.py:1860 read GUARDIAN_SECRET_KEK from os.environ. But compose guardian-updater.environment INTENTIONALLY doesn't pass it (per v0.5.51's env-block stability invariant — any change there forces guardian-updater recreate). So os.environ.get() returned empty.",
      "Latent since v0.5.51. Surfaced now because v0.6.x finally exercised the connector-container start path end-to-end.",
      "Fix: read GUARDIAN_SECRET_KEK from /host/.env via _host_env_get() — same pattern as GUARDIAN_VERSION / DIGEST_GUARDIAN_* reads. Preserves env-block stability invariant AND propagates KEK correctly.",
  ...
  },
  {
    ...
    date: "2026-05-19",
    title:
      "Bring per-connector images into the dev cycle. Pre-v0.6.16 the dev manifest pinned connector digests to the LATEST CUSTOMER RELEASE; build-connectors.yml could push fresh :dev tags but guardian-updater would still pull the stable digest. Same class of issue as v0.6.12 (guardian-updater not in dev cycle) — fixed for one image, repeated for seven.",
    highlights: [
      "Surfaced after v0.6.15: the new Caldera connector image with calderaUrl fallback was pushed to :dev, but when guardian-updater restarted the operator's instance container, it pulled the SAME old v0.6.4 digest. Caldera dispatch still failed.",
      "Root cause: build-dev-installer.yml's connector-digest resolution fetched from the latest stable release manifest, NOT from :dev tags. So /opt/guardian/connector-digests.env always carried customer-release digests.",
      ...
      "Net effect: changes to bundles/spark/connectors/<id>/src/* now flow through to guardian-vm. build-connectors.yml builds → dev manifest carries the digest → connector-digests.env gets it → next instance restart pulls the new image. Closes the dev-cycle gap for the last 7 images.",
  ...
  },
  {
    ...
    highlights: [
      "After v0.6.13 deployed (always-HTTPS): reconcile got 'SSL: CERTIFICATE_VERIFY_FAILED: self-signed certificate.' The call sites read GUARDIAN_TLS_VERIFY with default '1' → verify=True against a self-signed cert → never succeeds.",
      "Architectural fact: intra-cluster trust boundary is the docker network alias (only the legitimate guardian-agent container answers on that DNS name), NOT the cert chain. The cert exists to encrypt the wire, not authenticate the host. So verify=False is correct for the default compose-internal URL.",
      "Fix: new _agent_tls_verify() helper. Default URL → verify=False unconditionally. Operator-overridden GUARDIAN_AGENT_INTERNAL_URL → honors GUARDIAN_TLS_VERIFY env so CA-fronted deployments can opt-in to chain validation.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "So v0.6.11 always fell into the http branch on customer + dev installs. Hit the TLS proxy. Got the SAME 'Server disconnected without sending a response' error. Empirically verified: curl http://guardian-agent:8080 → Empty reply; curl -k https://guardian-agent:8080 → TLS handshake succeeds.",
      "v0.6.13 fix: _resolve_agent_internal_url() always returns https://guardian-agent:8080 (or the GUARDIAN_AGENT_INTERNAL_URL operator override). No more env-based derivation. The agent is HTTPS in v0.4.0+, period.",
      "Per CLAUDE.md § 'Rule 3 — Derive runtime state from observable evidence, not env vars that mid-process scripts mutate' (v0.4.0 retrospective): v0.6.11 used env (wrong). v0.6.13 uses an architectural fact (guardian-agent:8080 = always HTTPS).",
      ...
      "This unblocks POST /api/v1/connectors/reconcile + per-connector audit URL writes. Next: test reconcile on guardian-vm; recreate Caldera instance; verify caldera tools dispatch; run kill chain.",
    ],
  },
  {
    version: "0.6.12",
    date: "2026-05-19",
    title:
      "Bring guardian-updater into the dev cycle. Customer-release is no longer the only path to test updater changes — fixes the design gap that forced 'approve a customer release just to test untested updater code'.",
    highlights: [
      "Operator caught the design flaw during v0.6.11 testing: pre-v0.6.12 the only way to test guardian-updater code changes was to cut a customer release tag (which then triggered release.yml to build the image). That made customer releases the ITERATION mechanism, not the FINAL step. Violates the 'release only what's verified' principle.",
      ...
      "Fix: new .github/workflows/build-updater.yml mirrors build-agent.yml — triggers on updater/** source changes, builds + pushes guardian-updater:dev. build-dev-installer.yml's workflow_run cascade list adds 'Build updater'; the :dev digest resolution loop adds guardian-updater alongside agent/xlog/caldera; the 'fetch from latest stable' block removes guardian-updater (only browser remains there).",
      "Risk: symmetric to agent/xlog/caldera dev builds. A bug in updater could break the dev install on guardian-vm. Mitigation: customer installs are unaffected because release.yml still builds updater independently on tag push. The dev cycle gives us a chance to CATCH bugs before customers do.",
      "Unblocks v0.6.11 testing: guardian-updater's TLS-aware agent URL fix (line 1620 / 1763 / 2036) can finally be exercised on guardian-vm without a customer-release tag. Push triggers rebuild → auto-deploy → reconcile works → caldera recreate works → kill chain fires.",
    ],
  },
  {
    version: "0.6.11",
    ...
    highlights: [
      ...
      "Fix: new _resolve_agent_internal_url() helper. Derives scheme from observable state per CLAUDE.md § 'Rule 3' (v0.4.0 retrospective): honors GUARDIAN_AGENT_INTERNAL_URL env override if explicit, otherwise reads /host/.env's SSL_CERT_PEM (non-empty → https, empty → http).",
      "Three call-site replacements: instance container_url update (line 1620), reconcile instances fetch (line 2036), per-connector container's GUARDIAN_AUDIT_URL inheritance (line 1763).",
      "Customer upgrade: install v0.6.11 — guardian-updater gets the new helper. After install, reconcile works on TLS installs, per-connector audit writes route correctly. This is what unblocks the kill-chain end-to-end test: v0.6.7 added the connector-digests reader, v0.6.11 lets guardian-updater actually talk to the agent over TLS.",
    ],
  },
  {
    version: "0.6.10",
  ...
  },
  {
    ...
    highlights: [
      "Pre-v0.6.8 dev cycle: push → wait for build → operator manually runs `sudo /home/ayman/guardian-installer-dev` → operator pings agent → agent smokes. Three failure modes: (1) operator forgets to re-install, (2) version skew between deployed code and smoke-test target, (3) wasted operator time per push.",
      "v0.6.8 change: build-dev-installer.yml gains a new step after staging the binary. It runs `sudo /home/ayman/guardian-installer-dev` itself. The self-hosted runner IS guardian-vm running as ayman with NOPASSWD sudo (per docs/CICD.md § Install location) — no SSH, no extra credentials.",
      "Customer flow unchanged: customer-release tags still require explicit operator approval in chat. Customer installs still happen via `sudo /opt/guardian/guardian-installer` driven by the customer. Auto-deploy is a dev-cycle-only mechanic.",
  ...
  },
  {
    ...
    date: "2026-05-18",
    title:
      "Architectural cleanup — decouple per-connector image digests from /opt/guardian/.env per the operator config-file separation principle. Closes #55. Operator discovered during v0.6.6 chat testing that the Caldera connector container wasn't starting; root cause was a design violation where connector image refs (which aren't docker-compose substitutions) were being written to .env alongside service credentials.",
    highlights: [
      "The principle (now codified in CLAUDE.md § Operator config-file separation): .env is for service credentials + the 5 core compose-substitution digests that docker-compose interpolates. Per-connector image refs are NOT compose substitutions — they're runtime data guardian-updater uses to spawn dynamic instance containers. They live in a dedicated file at /opt/guardian/connector-digests.env.",
      "guardian-updater change: new reader _read_host_connector_digests_env() reads /host/connector-digests.env. _connector_digest() reads from there. Backward-compat: falls back to /host/.env for the transition window (operators upgrading from pre-v0.6.7), with a one-shot deprecation warning per process.",
      "guardian-installer change: applying the embedded manifest now splits writes — GUARDIAN_VERSION + 5 core stack-service digests + everything non-connector stays in .env; DIGEST_GUARDIAN_CONNECTOR_* lines route to the new connector-digests.env file. On upgrade from pre-v0.6.7 installs, the existing strip pattern removes legacy connector digests from .env automatically.",
      "build-dev-installer.yml change: the dev manifest now includes the 7 DIGEST_GUARDIAN_CONNECTOR_* values pulled from the latest customer release. Without this, dev installs leave operators without connector image refs and guardian-updater can't pull connector images (the bug that surfaced as 'no tool matched caldera_get_all_agents' during v0.6.6 testing).",
      "Customer upgrade: install v0.6.7 — installer applies the new file layout automatically. After install: `sudo ls -la /opt/guardian/connector-digests.env` (NEW file) + `sudo grep ^DIGEST_GUARDIAN_CONNECTOR_ /opt/guardian/.env` (should be empty). Recreate any connector instance via /instances and the container should start cleanly.",
  ...
  },
  {
    ...
    highlights: [
      "Bug: the guardian-installer's success message has told operators 'Future updates: use the Update button in the UI sidebar (no need to re-run this installer)' since pre-v0.5.20. That UI button was removed in v0.5.20 — there is no in-UI upgrade path; customer upgrades happen via the installer ONLY (per docs/CICD.md § v0.5.20 model correction). Stale message was misleading customers into looking for a button that doesn't exist OR skipping the installer at upgrade time and silently staying on stale code.",
      "Fix: replaced the 2-line stale block at installer/guardian-installer.template.sh:1105-1106 with ~10 lines of correct guidance — 'download the new guardian-installer binary from the GitHub Releases page and run it with sudo on this host. The installer detects the existing install at /opt/guardian and preserves your secrets, KEK material, and operator state across upgrades — only image digests + the docker-compose.yml are refreshed.' Includes the releases URL inline and the 'no in-UI Update button by design' rule.",
      "What does NOT change: installer behavior is identical (still detects /opt/guardian, preserves secrets, refreshes digest manifest in place). Only the closing message text changes. All other docs surfaces (/help/architecture, /help/user#upgrades, docs/CICD.md, docs/cicd-pipeline.svg) were already correct — this was the last live stale 'Update button' reference in the repo.",
      "Customer upgrade: install v0.6.5 — download the new guardian-installer binary from this release page and run with sudo. The corrected closing message will appear at the end of every subsequent install + upgrade.",
      "All other image digests (caldera / xlog / connectors / updater / browser) are byte-identical to v0.6.4 — only guardian-agent + guardian-installer change between v0.6.4 and v0.6.5. Closes #53.",
    ],
  },
  {
    version: "0.6.4",
    date: "2026-05-18",
    title:
      "Hotfix continuation — v0.6.3's fix is correct but the v0.6.0-v0.6.3 vanilla-retag chain needed a content-touch to break out. v0.6.4 finally ships Guardian plugin content to customers.",
    highlights: [
      ...
      "v0.6.4 fix: adds a marker tag (`v0.6.4-broke-vanilla-retag-chain`) to guardian-master-killchain.yml. The detection fires CHANGED=1, release.yml's v0.6.3 pull-from-:dev logic kicks in, and customers finally get a caldera image with actual Guardian content.",
      "Customer upgrade: install v0.6.4 guardian-installer. Open Caldera UI → Adversaries → confirm 'Guardian: Master Killchain' is visible (with the v0.6.4-broke-vanilla-retag-chain tag in its tags list, which is the marker that proves you're on a real Guardian-content image, not the vanilla chain).",
      ...
      "All other image digests (agent / xlog / connectors / updater / browser) are byte-identical to v0.6.3 — only guardian-caldera changes between v0.6.3 and v0.6.4. Same as the v0.6.0 → v0.6.1 → v0.6.2 → v0.6.3 chain.",
    ],
  },
  {
    version: "0.6.3",
    date: "2026-05-18",
    title:
      "Critical hotfix — FIRST customer release that actually ships Guardian plugin content in the caldera image. v0.5.57 through v0.6.2 all shipped vanilla aymanam-style Caldera.",
    highlights: [
      "If you installed v0.5.57 through v0.6.2, your caldera image is vanilla Caldera with NO Guardian adversaries. v0.6.3 finally ships the actual content. Download + install v0.6.3 to get Guardian Master Killchain / Ransack / Super Spy / Thief / Lateral Movement Sweep + CTID emu plans + phishing v3.3 dropper + T1518.001 modern PSh discovery.",
      ...
      "v0.6.3 fix: release.yml now pulls the :dev caldera image (which build-caldera.yml correctly builds with the Guardian + CTID overlay) and retags it as :vX.Y.Z. Single source of truth for caldera content. Falls back to vanilla rebuild only if :dev pull fails.",
      "Verification after install: open Caldera UI → Adversaries → confirm \"Guardian: Master Killchain\" + 4 other Guardian adversaries are visible. If missing, you're on a vanilla image — re-run the v0.6.3 installer.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "All other image digests (agent / xlog / connectors / updater / browser) are byte-identical to v0.6.1 — only guardian-caldera changes between v0.6.1 and v0.6.2.",
  ...
  },
  {
    ...
    date: "2026-05-18",
    title:
      "Guardian T1518.001 modern PowerShell discovery (closes #52) + dynamic Caldera-implant detection in the phishing dropper. Curated Ransack success 78.6% → 94.4% (Scenario 1).",
    highlights: [
      "Closes #52 — stockpile T1518.001 abilities (wmic AV + Get-WmiObject Firewalls) systematically fail on Win Server 2022. Replaced with Guardian multi-path discovery: Get-CimInstance SecurityCenter2 + Get-MpComputerStatus Defender + Get-Service/Get-Process filtering for known EDR names (Cortex XDR / CrowdStrike / Sentinel / Defender / Cylance / Sophos / Kaspersky / etc.). Validated: Cortex XDR cyserver enumerated by name on both lab hosts.",
      "Guardian: Identify Firewalls now uses Get-NetFirewallProfile + Get-NetFirewallRule (the original stockpile ability was mis-named — actually queried AV WMI). Returns per-profile Domain/Private/Public state + inbound/outbound rule counts. netsh fallback for hosts without NetSecurity module.",
      ...
      "Scenario 1 release — caldera image digest changes (new content baked in), agent + xlog + connectors retain v0.6.0 digests. Re-run the v0.6.1 guardian-installer. Storage backward-compatible. Volumes preserved.",
      "In Caldera UI — Guardian: Ransack (curated) + Guardian: Super Spy (curated) now show modern PowerShell discovery output with actual EDR product names. Phishing ability adapts to whatever name the operator uses for their sandcat binary.",
    ],
  },
  {
    version: "0.6.0",
    date: "2026-05-18",
    title:
      "Major release — Guardian is now a detection-validation platform end-to-end. New Master Killchain, XDR connector, phishing v3.1 dropper, curated adversary catalog (Scenario 2).",
    highlights: [
      "Guardian Master Killchain (curated, 22 steps) — flagship adversary. Combines the v0.5.57 phishing chain + Guardian decoy + PowerKatz. Validated against Cortex XDR: produces 60+ alerts across 7 MITRE tactics (Execution, Persistence, Privilege Escalation, Defense Evasion, Credential Access, Lateral Movement, C2). Use as the standard 'does my lab work' smoke test.",
      ...
      "Cortex XDR connector — chat agent now queries Cortex cases + issues + runs XQL queries directly. Type ^xdr_get_cases_and_issues / ^xdr_run_xql_query / ^xdr_get_incident_extra_data. Detection-validation feedback loop lives inside Guardian — no more jumping to the XDR console mid-test.",
      "Guardian-curated adversary catalog: Ransack, Super Spy, Thief, Lateral Movement Sweep, Master Killchain. Each bypasses a structural gap in stockpile adversaries (workgroup DNS PTR records, missing decoy files) so they dispatch on a clean lab. Selection guide in docs/caldera-release-plan.md.",
      ...
      "Bug fixes — 7 XDR connector instance creation bugs (v0.5.61-77 chain). Bug-family audit across caldera/xsiam/cortex-content (v0.5.80, CLAUDE.md Rule 7). Connector instance form widget vocabulary expanded to 11 types. Test Connection dirty-tracking fixed. Caldera image now reconciles guardian + ctid emu plugins at startup. /help/cicd page added with 6 SVG diagrams.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Same dev-cycle gap as v0.5.78: page only ships to the running install after operator re-runs guardian-installer-dev. The agent-side probe is build-side only until then.",
    ],
  },
  {
    version: "0.5.78",
  ...
  },
  {
    ...
    highlights: [
      "Discovered during v0.5.75 end-to-end smoke probe — the very first time the new CLAUDE.md 'Agent-side headless smoke' discipline fired. cortex-xdr functions used the 'xdr_' prefix from connector.yaml's functionPrefix, but guardian-connector-runtime's prefix-stripping logic only knew about guardian_<connector_id>_, <connector_id>_, and guardian_ patterns. None matched 'xdr_' for connector_id=cortex-xdr. Container registered tools as xdr_get_cases_and_issues, agent proxy called bare 'get_cases_and_issues', mismatch → 'Unknown tool' on every call.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Bug 2: agent passed the operator-typed instance display name verbatim to guardian-updater's path validation, which rejects spaces, slashes, accents via ^[a-zA-Z0-9_-]+$. Operator named their instance 'Cortex XDR' → updater rejected 'Cortex%20XDR'/start with HTTP 400. Fix: new _slug_instance_name helper in instances.py; the agent slugs the name before calling updater. Display name in storage stays original; only the docker container name + URL path use the slug.",
      "CLAUDE.md addendum — the meta-fix. Operator caught five basic bugs in a row at hands-on time (v0.5.67/70/73/74 + #48). Root cause was process: my 'headless smoke' had been API-shape checks only (tsc/lint/build), not actual end-to-end execution against guardian-vm. The CLAUDE.md ceremony said status:dev-built → agent runs headless smoke → status:ready-for-testing. I was skipping step 2.",
  ...
  },
  {
    ...
    date: "2026-05-17",
    title:
      "Three coupled connector-instance bugs uncovered during XDR end-to-end smoke (closes #46): api_key persisted to config not SecretStore; edit-dialog Test silent; guardian-updater missing cortex-xdr",
    highlights: [
      ...
      "Bug 3: updater/src/main.py KNOWN_CONNECTORS was missing 'cortex-xdr'. v0.5.61 introduced the XDR connector but missed this set — guardian-updater rejected start_connector_instance with HTTP 400, no per-instance container ever spawned, tool calls failed with 'container_url — guardian-updater hasn't started the container yet'. Same family as v0.5.67's manifest.yaml miss.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Guardian has two parallel connector catalogs: bundle catalogue in manifest.yaml drives install; synthetic card list in route.ts drives UI display. They MUST stay in sync. v0.5.61 only updated one.",
  ...
  },
  {
    ...
    date: "2026-05-17",
    title:
      "Entrypoint hook ensures guardian + emu plugins enabled across upgrades — fixes v0.5.65 follow-up (volume-shadow on caldera_conf)",
    highlights: [
      "v0.5.65 post-install smoke revealed: even with v0.5.65's plugin-pattern fix correctly baking guardian plugin code into plugins/guardian/data/, Caldera's entrypoint still didn't enable the plugin. Root cause: caldera_conf is ALSO a named volume that shadows the image's modified default.yml. Docker doesn't re-copy named-volume content on image rebuild — the volume's first-install default.yml is frozen forever.",
      "Fix: extend installer/docker-compose.yml's caldera entrypoint to reconcile the plugins list at startup. After the existing user/api_key merge, the script appends guardian + emu to cfg['plugins'] if missing, logs the change, then writes local.yml. Idempotent on subsequent restarts.",
      "Pattern parallels guardian-agent's v0.3.2 skills-bootstrap entrypoint (same marker-driven default-merge for /app/skills/). The two volume-shadow problems (data/ + conf/) get opposite fixes: data/ moved to plugins/<name>/ (outside mount), conf/ stays where it is + entrypoint reconciles at startup.",
      "Operators upgrading from any pre-v0.5.66 release see 'enabled plugin: guardian' + 'enabled plugin: emu' in docker logs caldera on the first post-upgrade restart. Subsequent restarts no-op. Volume preserves the operator's UI-created adversaries/abilities + the api key + the red user creds.",
  ...
  },
  {
    ...
    date: "2026-05-17",
    title:
      "Fix v0.5.57 'factory state' claim — Guardian Caldera content moved to plugins/guardian/data/ so it survives the caldera_data volume mount (closes #44)",
    highlights: [
      "Smoke-test of v0.5.58-64 discovered: running guardian-caldera's /usr/src/app/data/ contained NONE of the v0.5.57 kill-chain abilities, v0.5.63 CTID adversaries, or v0.5.64 lookalikes — because installer/docker-compose.yml mounts caldera_data:/usr/src/app/data as a named volume that shadows image-baked content at runtime. v0.5.57's 'factory state' claim was aspirational, never validated against a fresh install.",
      ...
      "Fix: new guardian plugin shell at bundles/spark/caldera-content/plugin/ (hook.py + VERSION.txt + __init__.py). build-caldera.yml overlay rewritten to target third_party/caldera/plugins/guardian/data/ instead of /data/. New workflow step inserts '- guardian' into conf/default.yml's plugins list (NOT local.yml — that's volume-mounted and absent in build context).",
      "Same path-correction applied to v0.5.63's broken emu-enable step (was looking for local.yml, now targets default.yml).",
      "Upgrade caveat: caldera_conf is ALSO volume-mounted, so existing installs with populated conf volumes need a one-time `docker exec ... sed -i '/^- manx$/a- guardian' /usr/src/app/conf/local.yml; docker restart caldera`. Fresh installs auto-enable. Follow-up v0.5.67+ entrypoint hook will automate the upgrade case.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "New adversary guardian-lookalike-validation chains all 5. Use to confirm Cortex detection coverage without operational compromise. Runtime ~3 min.",
  ...
  },
  {
    ...
    date: "2026-05-17",
    title:
      "CTID Adversary Emulation Plans baked into guardian-caldera image (APT29 / FIN6 / OilRig / menuPass / Sandworm / Carbanak) — closes #41",
    highlights: [
      ...
      "Graceful degrade: if CTID clone fails (transient network blip, repo URL moved), build continues with a warning rather than blocking. Emu plugin ships empty but stockpile + atomic + guardian adversaries still load.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Use case: operator says 'run the v0.5.57 guardian-killchain and tell me what Cortex caught'. Agent fires caldera_create_operation, polls to completion, queries xdr_get_cases_and_issues for the run window, builds per-ATT&CK-technique coverage matrix. The workflow that EPIC #39 codified — manual today via Claude in chat, autonomous in v0.7.x+ via guardian-agent + Gemini.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Fix: composite action .github/actions/build-and-push-dev-image/action.yml gains pull-policy input. Default 'always' keeps --pull for first-party Docker Hub bases (python:3.12-slim etc). 'never' skips it for builds whose FROM is a Guardian-own image just built locally in a prior job — the per-connector pattern.",
  ...
  },
  {
    ...
    date: "2026-05-16",
    title:
      "Caldera kill-chain content is factory state — baked into the guardian-caldera image, auto-loaded on container start. Plus 8 new noisy steps tuned for marquee XDR signals (LSASS comsvcs dump, Defender disable, Run-key + scheduled-task persistence, certutil decode, event-log clear).",
    highlights: [
      "v0.5.55 shipped the kill-chain YAMLs as a bundle; v0.5.57 makes them factory state. Fresh guardian-caldera installs see the adversary preloaded in the UI — zero manual import. The CI workflow (.github/workflows/build-caldera.yml) overlays bundles/spark/caldera-content/ into the caldera build context just before docker build runs; Caldera's data_svc auto-scans /usr/src/app/data/{abilities,adversaries}/* on container start.",
      ...
      "Detection narrative: marquee atomic alerts at steps 7 (LSASS), 9 (Defender tamper), 14 (lateral), 20 (Security 1102). Sequence alerts across steps 3-5 (discovery burst) and 11-13 (persistence trio). Forbidden going forward: dropping Guardian YAMLs into third_party/caldera/ (submodule trap); adding manual-import instructions to operator-facing docs (auto-load IS the path); auto-undoing Defender disable or event-log clear in cleanup (persistent state is part of the detection scenario).",
    ],
  },
  {
    version: "0.5.56",
  ...
  },
  {
    ...
    highlights: [
      "New directory bundles/spark/caldera-content/ with 9 ability YAMLs (organized by tactic 00-bootstrap through 11-exfiltration) + 1 adversary profile (Guardian phishing → ransomware kill chain, 12 steps). 6 abilities custom; 6 referenced from Caldera's stockpile by UUID.",
      ...
      "Two setup-only bootstrap abilities under 00-bootstrap/ configure the workgroup-auth knobs that make cross-host lateral actually work on Server 2022: LocalAccountTokenFilterPolicy=1, Service\\AllowUnencrypted, Client\\TrustedHosts, SMB firewall, guardianlab admin user. Run once each before the kill chain.",
      ...
      "Lab-safety constraints called out explicitly: no actual encryption (T1491 drops a note, doesn't encrypt), no real exfil (POSTs to internal Caldera listener, returns 500 — telemetry-only), no real malware (emailclient.exe is renamed signed notepad). Hardcoded creds (guardianlab / Lab26Pass#) chosen to satisfy Windows complexity AND avoid containing the username substring (a Windows password rule that bit our earlier iterations silently).",
      "Forbidden going forward: making any of these abilities singleton (kills repeatability); replacing the base64 wrap with plaintext (Caldera whitespace-strips it); changing the guardianlab password indirection (parameterizing forces runtime fact-injection which complicates install); bundling stockpile abilities (Caldera ships them; duplicating creates drift).",
    ],
  },
  {
    version: "0.5.54",
  ...
  },
  {
    ...
    date: "2026-05-16",
    title:
      "guardian-updater now reads GUARDIAN_VERSION + DIGEST_GUARDIAN_* from /host/.env at runtime (closes the unchanged-image-recreate invariant violation)",
    highlights: [
      "Pre-v0.5.51: guardian-updater was recreated on EVERY stack upgrade even when its own image digest was unchanged, because docker compose's config-hash flipped whenever any stack digest in its 'environment:' block flipped. v0.3.0+'s 'same digest = no recreate' invariant held for xlog + caldera but not guardian-updater.",
      "Fix: new /host/.env cached reader in updater/src/main.py with 30s TTL. Helpers _running_guardian_version(), _stack_digest(svc), _connector_digest(id) replace the 7 os.environ.get callsites. Cache is invalidated immediately by _apply_manifest_to_env so the updater's own manifest writes propagate without latency.",
      "Customer compose strips GUARDIAN_VERSION + 5 stack DIGEST_GUARDIAN_* + 6 connector DIGEST_GUARDIAN_CONNECTOR_* entries from guardian-updater's environment block. Stable env block means stable container config hash means no recreate on upgrades that don't touch guardian-updater's image.",
      "Operator-visible change: guardian-updater in-memory state (compose subprocess handles, ongoing-update SSE state, the .env cache itself) now survives across stack upgrades. Same behavior xlog + caldera already had.",
      ...
      "Forbidden going forward: re-adding DIGEST_GUARDIAN_* or GUARDIAN_VERSION to guardian-updater's environment: block (re-introduces the recreate cycle); reading these via os.environ.get anywhere in updater/src/main.py (grep test must return zero matches); removing the cache-invalidation hook in _apply_manifest_to_env (would briefly return stale values); reading TLS material or secrets from /host/.env (it's metadata, not a free-form key-value store).",
    ],
  },
  {
    version: "0.5.50",
    date: "2026-05-16",
    title:
      "Installer: freshly-prompted GHCR tokens now persist to /opt/guardian/.env (kills the re-prompt-every-run bug)",
    highlights: [
      "Pre-v0.5.50: on every re-run of /home/ayman/guardian-installer-dev with an expired .env-stored token, the installer prompted for a fresh PAT, validated it, used it for docker login + pulls — but NEVER wrote the fresh token back to .env. Next re-run hit the prompt again. Operators were re-pasting tokens on every install.",
  ...
  },
  {
    ...
    highlights: [
      "v0.5.31 + v0.5.44 + v0.5.47 shipped discovery + lifecycle for the entry-point plugin system; v0.5.48 closes the LAST piece: plugin-contributed handlers in the guardian.hooks group are now invocable from the agent's hook-runner via a new 'plugin' transport type in /settings/hooks.",
      ...
      "Plugin author contract: in pyproject.toml declare [project.entry-points.\"guardian.hooks\"] my-handler = \"my_pkg:my_handler\"; the function signature is def my_handler(payload: dict, config: dict) -> dict | None where None=no-op and dict={decision, reason} is a HookResult. Operator config is plugin-defined (no introspection from TS); plugin author documents their own contract.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Operator UX: paste spec → click Install → see success note → catalog refreshes. Click Uninstall on any row → confirm → row disappears. Restart guardian-agent for newly-installed package's CONTRIBUTED handlers to become callable; discovery alone doesn't import them (handler-invocation bridge ships in v0.5.48).",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Agent headless smoke discipline documented in docs/CICD.md: setup via GCP IAP tunnel (localhost:3001 → guardian-vm:3000), API smoke via curl over the SSH tunnel, UI smoke via Playwright / Chrome MCP against the tunneled UI. Result reporting includes per-bullet pass/fail, screenshots of verified surfaces, API response excerpts, any unexpected behavior tagged blocking or follow-up.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Plugin discovery: bundles/spark/mcp/src/main.py adds log_discovery() call after register_bench_routes. Walks all 5 reserved groups (guardian.skills, guardian.connectors, guardian.hooks, guardian.scanners, guardian.providers) and logs per-group counts. Wrapped in try/except so a discovery failure doesn't break MCP boot.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "List page: shows run_id + manifest_id + router_preset + 5 per-row metrics (correctness %, avg Jaccard, cost p50, wall p50, cases clean/total). Run-now form: manifest input (bundled id like guardian-soc-v1 or full path) + optional router-preset model override + Run Benchmark button. Synchronous trigger — returns when run completes.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Sample 3-case corpus at bundles/spark/mcp/src/usecase/bench_cases/guardian-soc-v1.yaml. Cases: generate-syslog (1 tool call expected — guardian_create_data_worker), list-scenarios (1 — guardian_list_scenarios), conversational-no-tool (0 tools). Each has prompt + expected_output_match (substring) + expected_tool_calls + max_wall_seconds.",
      "New MCP tool bench_run(manifest, router_preset_model?, thinking_enabled?) — approval-gated risk_tier=soft. Agent invokes from chat ('run the guardian-soc-v1 benchmark'). Returns {run_id, summary}. Registered in _BUILTIN_LEGACY_TOOLS in connector_loader.py.",
      "Operator usage: 'Run the guardian-soc-v1 benchmark' in chat → agent calls bench_run → 3 dispatches → scored summary in chat. Persisted run inspectable via docker exec guardian_agent sqlite3 /app/data/benchmark_runs.db.",
      "Still deferred: /observability/bench UI page (run history + compare + drill-down); CLI binary guardian bench run; larger curated corpus + val/test split; scheduled bench job; regression-flag release-gating integration.",
      "Forbidden going forward: skipping infrastructure-error exclusion in scoring; mutating guardian-soc-v1.yaml corpus without a baseline-rebaseline plan (corpus IS the SLO contract — add new corpora rather than mutate this one).",
    ],
  },
  {
    version: "0.5.32",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Five reserved group names: guardian.skills, guardian.connectors, guardian.hooks, guardian.scanners, guardian.providers. PluginRef dataclass identifies group + name + dist_name + dist_version + target. discover_plugins(group) walks importlib.metadata.entry_points(group=...). discover_all() across all five. log_discovery() emits boot-time logs + returns per-group counts for telemetry.",
      "Third-party contract — in your pyproject.toml: [project.entry-points.'guardian.skills'] my-skill = 'my_pkg.skills:my_skill_factory'. The agent walks these at boot.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Forbidden going forward: auto-installing the hook without operator opt-in (Guardian doesn't ship hooks the operator didn't ask for); silent compaction in any future chat-route refactor (PreCompact MUST fire so the registered hook works).",
    ],
  },
  {
    version: "0.5.24",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Pre-release / beta channel (NEW top-level future-work section): design sketch for when we want trusted-beta-customer access — vX.Y.Z-beta.N tag pattern, prerelease + NOT-latest marking, guardian-updater channel-awareness. Status today: not implemented; defer until first beta-program demand.",
      ...
      "Self-hosted runner capacity (NEW subsection in guardian-vm runner prerequisites): current state (one runner), when to add a second (queue depth >3 jobs for >5 min, recurrent operator-reported delays), topology options (same-VM vs separate-VM), failover plan if guardian-vm goes down.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "guardian-updater in the release loop (NEW top-level section): the architectural piece previously underspecified. Source location, MCP_TOKEN auth + PAT auth for GHCR pulls, 5 API endpoints, update detection mechanics (GitHub Releases API polling + manifest fetch + per-service digest comparison), V1 scope constraints (no rollback, no volume snapshots, no self-update), which 3 services updater manages, scenario-aware behavior.",
      ...
      "Monorepo release invariant (NEW subsection): all 11 images (5 stack + 6 connector) ship in lockstep at the same vX.Y.Z. Mixed-version manifests are not supported by guardian-installer or guardian-updater. Codified to prevent the next person from trying to violate it.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "First install attempt against v0.5.9's dev-latest prerelease hit this — operator saw 'Token from /opt/guardian/.env failed validation against ghcr.io' with a fresh, valid read:packages PAT. Direct GHCR probe with the same token from a workstation returned HTTP 200 for :dev. The Accept header was the only thing different.",
  ...
  },
  {
    ...
    highlights: [
      "Pre-v0.5.9 the operator's dev install on guardian-vm failed at docker compose pull even when v0.5.8's registry-token probe succeeded: a PAT with read:packages (and even repo + read:packages) returned 404 for ghcr.io/kite-production/guardian-agent:dev while :vX.Y.Z worked. Root cause: GHCR enforces pull access per IMAGE VERSION, not per package or token scope alone.",
      ...
      "The fix: build-dev-installer.yml publishes a GitHub Release marked --prerelease with tag dev-latest after each successful dev build. The :dev image versions inherit pull permission from this prerelease, so the operator's PAT (the same kind customers use, with read:packages) can now pull :dev from guardian-vm.",
      ...
      "Operator-facing commands unchanged: git push origin main → wait for build-dev-installer.yml → sudo /home/ayman/guardian-installer-dev on guardian-vm. The fix is entirely in the access layer — what was a confusing 404 at pull time becomes a clean install.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "If no TTY is available (non-interactive run, e.g. scripted automation), the installer dies with a clear message: 'provide GUARDIAN_REGISTRY_TOKEN=… or update .env's GUARDIAN_REGISTRY_TOKEN line'. No silent failure.",
  ...
  },
  {
    ...
    highlights: [
      "Operator-driven design correction: the dev workflow (formerly deploy-dev-installer.yml) used to run sudo ./guardian-installer-dev on guardian-vm after every successful image build, automatically installing the new dev bytes. v0.5.7 retires that behavior. The workflow is renamed build-dev-installer.yml and only BUILDS + stages the dev installer binary; the operator drives every install on guardian-vm.",
      "Why retire auto-deploy: doesn't match the customer flow (customers download guardian-installer, run it manually), takes upgrade timing away from the operator, and short-circuits explicit upgrade-from-prior-version testing. The new contract: CI builds + stages the binary at /home/ayman/guardian-installer-dev on guardian-vm + uploads as workflow artifact. Operator runs sudo /home/ayman/guardian-installer-dev when ready to test.",
      "Workflow rename: deploy-dev-installer.yml → build-dev-installer.yml. Job 'Deploy on guardian-vm' replaced with 'Stage dev installer at /home/ayman/guardian-installer-dev'. Atomic stage via cp + mv so the operator can't observe a partial binary even if the workflow is in flight when they run the installer.",
      ...
      "Operator's new dev cycle: git push origin main → per-service builds rebuild changed images → build-dev-installer.yml stages fresh binary at /home/ayman/guardian-installer-dev → operator SSH's to guardian-vm + runs sudo /home/ayman/guardian-installer-dev → smoke-test via browser → ASK FOR APPROVAL → git tag vX.Y.Z → release.yml fires.",
      "Customer impact: none. Customers were never affected by the dev auto-deploy path. Customer flow has always been 'download guardian-installer, run it' and remains unchanged.",
      "Forbidden post-v0.5.7: re-introducing an auto-deploy step in build-dev-installer.yml or any successor workflow (CI builds, operator installs — full stop). Adding a 'auto-install on workflow_dispatch' bypass (the operator dispatches the build manually if they want a fresh installer; they still run the install themselves). Pushing fixes-that-bypass-installer onto guardian-vm via tar+scp or sshpass git pull (the dev-installer IS the install path; if it can't install something, fix the installer, not work around it).",
    ],
  },
  {
    version: "0.5.6",
    ...
    highlights: [
      "Operator-reported: sudo /opt/guardian/guardian-factory-reset --dry-run printed a warning and prompted for typed 'YES' to proceed without auto-recovery. Two root causes: (1) the installer binary was never persisted into $INSTALL_DIR/ after install — it lived wherever the operator ran it from (~/, /tmp/, runner workspace). After install, /opt/guardian/ had docker-compose.yml + .env + the two recovery scripts but not the installer that produced them. (2) Factory-reset offered a 'proceed without auto-recovery' workaround. That's the wrong shape: the installer is part of the shipped package; its absence is an install-integrity violation, not a bypass scenario.",
      "Installer self-installs to $INSTALL_DIR/ at the end of every successful install. After the stack-healthy check (step 7 epilogue), the installer copies its own binary to $INSTALL_DIR/$(basename $0). The basename preserves the flavor: guardian-installer-dev lands at /opt/guardian/guardian-installer-dev; guardian-installer lands at /opt/guardian/guardian-installer. Idempotent on re-run; fails-soft if the copy can't happen (the rest of the install already succeeded).",
  ...
  },
  {
    ...
    security: true,
    highlights: [
      "Operator-stated requirement: no credentials in any Guardian image, full stop. Pre-v0.5.5 audit found exactly one violation — the literal 'guardian-admin-CHANGE-ME' in mcp/agent/lib/auth-defaults.ts:65, baked into the guardian-agent image and passed to the entrypoint's seed routine. v0.5.5 closes the gap.",
      "Installer auto-generates a random per-install bootstrap password (openssl rand -base64 24, 24 chars) into GUARDIAN_DEFAULT_ADMIN_PASSWORD in /opt/guardian/.env. The agent's entrypoint sources it from env at seed time. The seed path fails-loud if the env var is empty on a fresh install — refuses to seed an empty hash; operator must re-run the installer (which auto-generates) or use sudo /opt/guardian/guardian-reset-admin-password to set credentials interactively.",
      "The DEFAULT_ADMIN_PASSWORD constant is gone from auth-defaults.ts. ADMIN_USERNAME, SESSION_COOKIE_NAME, SESSION_TTL_SECONDS stay — usernames aren't credentials, cookie names aren't secrets, TTLs are constants. Audit shape post-v0.5.5: docker exec guardian_agent grep -r guardian-admin-CHANGE-ME /app → no matches.",
      "Backward-compatible upgrade. Existing v0.4.0–v0.5.4 customers upgrading to v0.5.5: SecretStore already holds the operator's password hash; seed returns 'already_initialized'; env var ignored. On the next installer run GUARDIAN_DEFAULT_ADMIN_PASSWORD gets back-filled into .env (harmless — only consulted if SecretStore is ever wiped). Fresh installs of v0.5.5 boot with a per-install random password printed in three places: installer epilogue banner, agent's first-boot docker logs banner, and /opt/guardian/.env.",
      "Lost the random value before completing the forced first-login change? Run sudo /opt/guardian/guardian-reset-admin-password from the host (v0.5.3+ utility). The factory-reset path also restores access: guardian-factory-reset wipes SecretStore but preserves .env, so the post-reset seed uses whatever value is in .env (back-filled by the installer, never wiped).",
      ...
      "Forbidden post-v0.5.5: reintroducing any hardcoded credential into any Guardian image (auth-defaults.ts, entrypoint.sh, or any source file we control). Adding a 'silent fallback' to the literal guardian-admin-CHANGE-ME if GUARDIAN_DEFAULT_ADMIN_PASSWORD is empty (the fail-loud is the contract — silent fallbacks landed us in v0.3.x regressions). Storing GUARDIAN_DEFAULT_ADMIN_PASSWORD anywhere other than .env (it's not in SecretStore because the SecretStore consumes it during init; it's not in the image because that's the whole point of v0.5.5).",
    ],
  },
  {
    version: "0.5.4",
    ...
    highlights: [
      "Operator-reported. cat /opt/guardian/.env on a guardian-vm with ~25 dev-installer runs showed ~25 stale '# ─── Digest manifest (managed by guardian-installer vdev-XXXXX) ──' header blocks accumulated at the bottom of the file. Only the last block had its values; the rest were orphan headers. Pre-v0.5.4 the strip step targeted only ^GUARDIAN_VERSION= and ^DIGEST_GUARDIAN_ lines; the 3–4 line comment header above them was untouched, so every re-run added a fresh header without removing the old ones.",
      "Fix shape: extended the sed strip patterns in both installer paths (installer/guardian-installer.template.sh and installer/install.sh) to also delete the comment header lines. Matches the canonical 4-line block emitted by guardian-installer AND the 3-line variant install.sh emits, so a .env that's been touched by both flavors gets cleaned the same way. Added an awk pass after the strip to collapse consecutive blank lines, so the cleanup of N orphan headers doesn't leave N blank gaps in the file.",
  ...
  },
  {
    ...
    date: "2026-05-13",
    title:
      "Host-side recovery utilities — guardian-factory-reset + guardian-reset-admin-password",
    highlights: [
      "Operator-driven. v0.5.2 fresh-state hygiene exposed a related pain point: the only way to test a fresh install was to manually type the wipe-and-reinstall recipe from CLAUDE.md, and the only way to reset a forgotten password was to memorize a docker-exec one-liner. Both work, neither is friendly. v0.5.3 ships these as two named host-side scripts that mirror each other's invocation shape: sudo /opt/guardian/guardian-<utility>.",
      "guardian-factory-reset. Host-side script that returns Guardian to its fresh-shipped state. Lists every guardian_* docker volume (with approximate size), asks the operator to type 'FACTORY RESET' as a typed-confirmation guardrail, stops the stack, wipes the volumes, and re-runs the installer so the recovery brings containers back up. Preserves /opt/guardian/.env so GUARDIAN_SECRET_KEK + registry credentials survive across the reset — re-installs are fast (no image pulls). Flags: --yes (skip prompt, for scripted use), --dry-run (show plan without wiping), --help. Host-side by physical necessity: a container can't delete the docker volume it's mounting (the daemon refuses with 'volume in use'), so factory reset MUST operate from outside the container boundary.",
      "guardian-reset-admin-password. Host-side wrapper around the in-container CLI at /app/cli/reset-admin.mjs. v0.4.0 deliberately put the credential-write logic inside the agent image (SecretStore + audit machinery, same code path as /profile's change-password flow); v0.5.3 adds the host wrapper for operator-UX consistency with guardian-factory-reset. The wrapper validates the agent container is running, then exec-replaces itself with the docker-exec invocation. Credential-write logic stays inside the container — single source of truth, no parallel host-side implementation (the v0.4.0 retrospective rule).",
      "Both scripts ship in BOTH installer distribution paths. The single-file guardian-installer binary embeds them via heredoc (same pattern as docker-compose.yml — see installer/build-guardian-installer.sh's __INSTALLER_FACTORY_RESET_SH__ + __INSTALLER_RESET_PASSWORD_SH__ markers). The multi-file install kit copies them verbatim via release.yml's 'Pack customer install kit' step. After install, they live at /opt/guardian/guardian-factory-reset and /opt/guardian/guardian-reset-admin-password — chmod 755, ready to invoke under sudo.",
      ...
      "Build pipeline. installer/build-guardian-installer.sh gained FACTORY_RESET_SH + RESET_ADMIN_SH env vars (with defaults pointing at the installer/ paths). release.yml's 'Build guardian-installer' step inherits the defaults from the working directory. deploy-dev-installer.yml's 'Build guardian-installer-dev' step inherits the same way. No workflow changes needed beyond the install-kit packing step.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Skills page workspace cleanup. Removed two sections from the skill-detail panel that had no backend wiring: 'Workspace Assignment' (toggle workspaces on/off per skill) and 'Workspace Overrides' (per-workspace override config). Both ported over from Spark's multi-tenant skill UI; Guardian is single-tenant so they rendered a permanent decorative placeholder ('All workspaces using platform defaults') for every skill forever. Removed the WsToggle component, the workspaces and overrides fields from the SkillDef interface, and the corresponding stub entries from all 20+ hardcoded fallback skill cards.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "User-uploaded connectors + system/user distinction. POST /api/v1/marketplace/upload accepts a multipart connector.yaml, validates against schema, rejects bundle-id collisions + existing-user-id collisions, requires an 'image' field (the OCI reference to the user's pre-published connector container), writes to /app/data/user_connectors/<id>/. DELETE /api/v1/marketplace/<id> deletes user connectors (with instance-count 409 protection); bundle connectors are 403-rejected (image-baked, undeletable). guardian-updater extended to pass image_ref through to docker.run so user connectors don't need to be in KNOWN_CONNECTORS.",
      ...
      "Customer impact on upgrade: existing v0.4.x customer installs carry their state in the guardian_data volume. v0.5.0's upgrade migration runs once at boot, iterates instances.db, and auto-installs every connector with existing instances (origin='bundle'). Customer experience: no change — the 3 connectors they were using (typically caldera/xsiam/xlog) stay installed, their instances stay live, their tools stay registered. The marketplace page now shows them as 'installed' with the auto-installed timestamp. Volume-wipe case boots cleanly into the 'zero instances, 6 available connectors' default state.",
      "Forbidden post-v0.5.0: the pre-v0.5.0 marketplace_installs.json file (do not write to it — the MCP route is the canonical surface); runtimeMapping.style: 'module' or 'class' (deleted from the schema enum, validator rejects); _AUTO_MIGRATION-style env-var-to-instance auto-creation (the explicit install + instance create flow is the contract); hardcoding new connectors into guardian-updater's KNOWN_CONNECTORS set (use the image_ref body field for user uploads).",
    ],
  },
  {
    version: "0.4.0",
    ...
    highlights: [
      ...
      "Login UI port from spark-platform. components/auth/login-screen.tsx replaced wholesale with the spark login page per operator direction ('use the exact components and animations'). Animated WavyBackground (simplex-noise) + decorative robot character on the left half + 3-column form|divider|description grid + animated cyan glow divider + FlippingText typewriter cycling Guardian-relevant tool names. Branding swapped to Guardian-themed copy ('Cortex XSIAM, MITRE ATT&CK, CALDERA, Vertex AI, Gemini, MCP' / 'Continuous SOC simulation').",
      "Reset CLI for the forgot-password case. New mcp/agent/cli/reset-admin.mjs (host-side wrapper at scripts/reset_admin.sh): docker exec guardian_agent node /app/cli/reset-admin.mjs prompts for new password (terminal echo masked) after a 'Type RESET' ceremony, writes to SecretStore, revokes all sessions, audits as actor=cli:<hostname>. Operator restarts the agent container to flush in-memory caches.",
      "Per-service CI builds (lands in the same release). Monolithic build.yml (608 lines) replaced with four focused workflows + composite action: build-xlog.yml, build-agent.yml, build-caldera.yml (path-filter triggers — only changed services rebuild), deploy-dev-installer.yml (workflow_run on any build OR push to installer/**). Two installer binaries from the IDENTICAL script body — only the digest manifest differs. Customer installer = guardian-installer (release.yml, semver digests). Dev installer = guardian-installer-dev (build-flow workflows, :dev digests). Caldera in particular benefits: its container state on guardian-vm now survives pushes that only touched the agent.",
      "Installer cleanup. Deleted installer/reset-ui-password.sh (358 lines, obsolete — replaced by the image-baked reset-admin.mjs). guardian-installer.template.sh: smarter sudo elevation (skips re-exec when INSTALL_DIR writable + docker reachable); post-install banner shows default credentials + change-at-profile flow + forgot-password CLI; old setup-wizard messaging removed. Installer accepts OUTPUT_NAME env var so build-guardian-installer.sh produces 'guardian-installer-dev' for the dev variant.",
      "Customer impact on upgrade: existing v0.3.x customer installs carry their state in the guardian_data volume. v0.4.0 boot finds the existing password hash in SecretStore and skips default-seeding — login continues to work with the customer's existing credentials. Old setup.json / .env.generated / .setup_complete files are NOT deleted by v0.4.0; they're simply never read. Volume-wipe case (operator drops the volume) boots cleanly into the default-credentials path with the post-install banner.",
      ...
      "Forbidden post-v0.4.0: invoking docker compose up -d / down -v / force-recreate over the IAP tunnel for testing or deploy. The guardian-installer is the contract per CLAUDE.md's 'Local-mirrors-customer deploy flow' rule. Volume management (e.g. wiping for a clean-break test) remains a manual operator step using docker volume rm — installers never touch volumes.",
    ],
  },
  {
    version: "0.3.27",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Migration impact: PATCH release. NO runtime code paths change — only the help pages render new content. SELECTIVE recreation: only guardian-agent rebuilds; other services retag from v0.3.23 with identical content digests. Backwards-compatible: existing deep links to /help/architecture#setup-wiring and /help/user continue to resolve; new content is purely additive.",
    ],
  },
  {
    version: "0.3.23",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Same FIELDS_TO_STRIP as v0.3.20 (UI creds, Vertex creds, connector secrets, GEMINI_MODEL, defaultLogFormat, GUARDIAN_TLS_VERIFY) plus XLOG_URL (env-only legacy; InstanceStore is now canonical).",
      ...
      "After v0.3.22 deploys + next login on guardian-vm: ls /app/runtime/ shows ONLY .setup_complete (25 bytes — the canonical signal). UI_PASSWORD, full SA JSON, all connector creds have no on-disk plaintext home anywhere; they live exclusively in SecretStore/ProviderStore/InstanceStore/settings_store with KEK-backed AES-GCM at rest.",
      "Migration impact: PATCH release. SELECTIVE recreation — guardian-agent rebuilds; other services retag from v0.3.21 with identical content digests. Rollback safe: entrypoint sources .env.generated only if file exists; missing file falls through to docker env defaults; stores remain populated.",
    ],
  },
  {
    version: "0.3.21",
    ...
    highlights: [
      ...
      "Added GUARDIAN_TLS_VERIFY to FIELDS_TO_STRIP. It's an env-managed runtime knob (set via docker env, NOT via the setup form); the legacy setup.json copy was dead data. The agent's existing fallback in getEffectiveRuntimeConfig reads it from process.env directly.",
      ...
      "After v0.3.21 deploys: next successful login on guardian-vm migrates remaining GUARDIAN_TLS_VERIFY, deletes setup.json, ls /app/runtime/ shows only .setup_complete. Idempotent re-runs find no file → quiet no-op.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Migration impact: test-only — no runtime code paths change. Image rebuild identical to v0.3.18. SELECTIVE recreation: only guardian-agent rebuilds with the new test file; other services retag from v0.3.18 with identical content digests.",
    ],
  },
  {
    version: "0.3.18",
    ...
    highlights: [
      ...
      "Migration impact: test-only — no runtime code paths change. Image rebuild identical to v0.3.17. SELECTIVE recreation: only guardian-agent rebuilds with the new test file; other services retag from v0.3.17 with identical content digests.",
    ],
  },
  {
    version: "0.3.17",
    ...
    highlights: [
      ...
      "guardian_cortex_content_index_runs_total{pack=<name>,result=succeeded|partial|failed} — one inc per index_kb call. partial = some indexed, some errored; failed = errors with zero successful indexes. Operators alert on result=failed aggregations and use the pack label to rank flaky packs.",
      "guardian_cortex_content_indexed_docs_total{action=insert|update|unchanged} — one inc per document inside a pack-index call. insert/update mean a Vertex embedding was generated; unchanged means source_hash dedupe skipped the embed step. Splits new-content rate from cache-hit rate.",
      ...
      "Migration impact: PATCH release. SELECTIVE recreation — only guardian-agent rebuilds; other services retag from v0.3.16 with identical digests. Backwards-compatible: new metrics appear in /api/v1/metrics as 0-valued counters immediately post-upgrade.",
    ],
  },
  {
    version: "0.3.16",
    ...
    highlights: [
      ...
      "Migration impact: PATCH release. No runtime code paths change — only the /help/architecture page renders new content. SELECTIVE recreation: only guardian-agent rebuilds; other services retag from v0.3.15 with identical digests. Backwards-compatible: existing deep links continue to resolve; new SubSections are purely additive.",
    ],
  },
  {
    version: "0.3.15",
    ...
    highlights: [
      ...
      "guardian_batch_proposals_total{approved=true|false} — every agent_batch_propose call increments once. Reveals batch usage AND the deny-rate over time.",
      "guardian_batch_actions_total{tool=<name>,result=success|fail} — one inc per individual action inside a batch. The tool label reveals which tools dominate batch traffic; result splits success vs failure for per-tool reliability dashboards. An action returning {error: ...} counts as fail, distinguishing per-tool flakiness from per-tool usage.",
      "guardian_batch_size — histogram of batch sizes with custom count-buckets (1, 2, 3, 5, 10, 25) matching the v0.3.10 25-action cap. Observed on both approve and deny paths (a denied 3-action batch is meaningful telemetry). Lazy-registered on first call since manifest pre-registration only handles counters.",
      ...
      "Migration impact: PATCH release. SELECTIVE recreation — only guardian-agent rebuilds; other services retag from v0.3.14 with identical digests. Backwards-compatible: new metrics appear in GET /api/v1/metrics as 0-valued counters immediately post-upgrade. No API shape changes, no required operator action.",
    ],
  },
  {
    version: "0.3.14",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Migration impact: test-only — no runtime code paths change. Image rebuild identical to v0.3.11 (tests run inside the agent image during the build's test-mcp-server job, which is the same image that gets deployed). SELECTIVE recreation: only guardian-agent rebuilds with the new test file; other services retag from v0.3.11 with identical digests.",
    ],
  },
  {
    version: "0.3.11",
    ...
    highlights: [
      ...
      "Migration impact: PATCH release. SELECTIVE recreation — only guardian-agent rebuilds; xlog/caldera/browser/updater retag from v0.3.10 with identical digests. Backwards-compatible: v0.3.10 batches (built-in-only) work identically; pre-v0.3.11 callers that referenced non-builtin tools got a clean error envelope, v0.3.11 callers get the action executed. The UI render is unchanged — connector tool names appear in the action list with the same row format as built-ins.",
    ],
  },
  {
    version: "0.3.10",
    ...
    highlights: [
      ...
      "Migration impact: PATCH release. SELECTIVE recreation — only guardian-agent rebuilds; xlog, caldera, browser, updater retag from v0.3.9 with identical digests. Fully backwards-compatible: the agent doesn't have to use agent_batch_propose; single-action approval continues unchanged.",
    ],
  },
  {
    version: "0.3.9",
    ...
    highlights: [
      ...
      "Migration impact: PATCH release. SELECTIVE recreation — only guardian-agent rebuilds (embeds the updated connector + marketplace entry); xlog, caldera, browser, updater retag from v0.3.8 with identical digests. Backwards-compatible: the 9 existing cortex-content tools are unchanged; index_kb is purely additive.",
    ],
  },
  {
    version: "0.3.8",
    ...
    highlights: [
      ...
      "Migration impact: PATCH release. SELECTIVE recreation — only guardian-agent rebuilds (entrypoint.sh + docstrings); xlog, caldera, browser, updater retag from v0.3.7 with identical digests. No operator action required.",
    ],
  },
  {
    version: "0.3.7",
    ...
    highlights: [
      ...
      "Migration impact: PATCH release. SELECTIVE recreation per the v0.3.0 digest-pinning architecture — only guardian-agent rebuilds (embeds the new cortex-content connector + SecretStore change + UI fixes + manifest update + docstring audit). xlog, caldera, browser, updater retag from v0.3.6 with identical content digests. One operator-action requirement for manual deploys without GUARDIAN_SECRET_KEK: either generate a KEK or set GUARDIAN_SECRET_KEK_ALLOW_PLAINTEXT=1; installer flows never need this.",
    ],
  },
  {
    version: "0.3.6",
    ...
    highlights: [
      ...
      "Migration impact: PATCH release. SELECTIVE recreation per the v0.3.0 digest-pinning architecture — only guardian-agent rebuilds; xlog, caldera, browser, updater retag from v0.3.5 with identical content digests. caldera state, xlog streaming workers, browser cache, updater state all preserved. The new schema index runs on first boot in-place; no data migration, no operator action.",
    ],
  },
  {
    version: "0.3.5",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Migration impact: PATCH release. SELECTIVE recreation per the v0.3.0 digest-pinning architecture — guardian-agent rebuilds (it embeds the new api/skills.py REST module + main.py wiring + Next.js proxy refactor + page.tsx displayName fix + the original parameter-rename fix in self_mod_tools.py); xlog, caldera, browser, updater all retag from v0.3.3 with identical content digests. caldera red-team operation state, xlog streaming workers, browser cache, updater state all preserved. In-flight chat sessions are dropped on the guardian-agent recreation but skills + jobs + connectors persist (they're volume-backed).",
    ],
  },
  {
    version: "0.3.3",
    ...
    highlights: [
      "Targeted patch to the password-reset recovery tool. Pre-v0.3.3 the script defaulted to a hardcoded username 'guardian' — fine as a documentation placeholder, wrong for any install whose UI_USER was set to something else during setup wizard. Result was a silent-bug class: reset wrote a hash for 'guardian', login failed for the actual operator (e.g. 'ayman') with 401 because the agent's login route enforces single-user mode by rejecting any username that doesn't match the configured UI_USER BEFORE checking the password hash. Audit event landed for the wrong user; failure looked like a typo. Surfaced during v0.3.2 smoke testing on a real customer-state install (bupa-engine).",
      "Fix: layered username resolution. Order: (1) --user explicit override, (2) UI_USER from /app/runtime/setup.json inside the container — same file the agent's auth code reads, so script + verify path always agree by construction, (3) UI_USER from container env (legacy fallback), (4) hardcoded 'guardian' (last resort, warned about loudly because it almost never matches a real install's configured operator).",
      ...
      "Updated header docstring with a 'Username resolution (v0.3.3+)' section documenting the resolution order + why pre-v0.3.3 'guardian' was wrong + the architectural reason single-user enforcement happens at the Next.js login route layer (the MCP-side hash store IS multi-user-ready).",
      "Migration impact: NO image rebuilds. The reset script is shipped as (a) a heredoc embedded in the guardian-installer binary, (b) a standalone GH Release asset, (c) a file in the install kit tarball. release.yml regenerates all three with the fixed script. Customers downloading v0.3.3+ get the fix automatically; operators with existing v0.3.x installs can either re-run the v0.3.3 guardian-installer (refreshes /opt/guardian/reset-ui-password.sh) or drop the standalone asset in place. Existing passwords are NOT affected — fix only applies to future invocations of the reset tool.",
      "Selective recreation: v0.3.2 → v0.3.3 hop only regenerates the guardian-installer binary; xlog/caldera/agent/browser/updater all retag from v0.3.2 with identical content digests per the v0.3.0 digest-pinning architecture. Effectively zero-downtime for customer in-memory state.",
    ],
  },
  {
    version: "0.3.2",
    ...
    highlights: [
      "Skills auto-merge: entrypoint.sh §1 now uses a per-release marker file (guardian_mcp_skills/.seeded_version) to drive automatic merges of new image-baked default skills into existing customer volumes. Pre-v0.3.2, new releases' skills never propagated to populated volumes — operators had to docker exec cp -r /app/mcp/skills-default/* /app/skills/ manually after every upgrade. v0.3.2 makes this automatic on first boot of each new release. The merge is still MERGE-not-REPLACE: customer-created skills + retired-but-on-disk legacy skills stay. Operator deletions of default skills stick across same-version restarts; they only re-introduce on the NEXT release upgrade (which is the right semantic — upgrading is opting into the new release's defaults).",
      "FORCE_SKILLS_SYNC=1 finally works on customer installs. v0.2.5 wired the env-var into the dev compose at the repo root, but never propagated to installer/docker-compose.yml. Customers using the documented `FORCE_SKILLS_SYNC=1 docker compose up -d --force-recreate guardian-agent` operator override hit a silent no-op. v0.3.2 adds the bare-name forwarding to the guardian-agent environment: block in the customer compose. With the marker-driven auto-merge above, this override is now rarely needed; primary use case is 'operator deleted a default skill on v0.3.x and wants it back without waiting for the next release'.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Connector is module-style (in-process Python). The upstream scripts are stdlib-only (urllib + json + re), so zero extra Python deps and no daemon needed. The connector layer adds a thin SystemExit-translation wrapper so the upstream scripts' sys.exit(1) HTTP-error paths produce structured {ok: false, error} returns instead of taking down guardian-agent on a transient docs-API blip. 15 unit tests cover the wrapper contract; live-API smoke during development returned canonical dedup/filter/arrayindexof topics with full URLs in <2 seconds.",
      ...
      "First release that exercises the v0.3.0 digest-pinning pipeline for a brand-new connector image. release.yml's per-connector image build matrix gained guardian-connector-cortex-docs; the manifest gained DIGEST_GUARDIAN_CONNECTOR_CORTEX_DOCS; the customer compose forwards the new digest to guardian-updater. Customers upgrading v0.3.0 → v0.3.1 see SELECTIVE recreation: guardian-agent (which embeds the new connector code + skill) is the only stack-tier service that recreates; xlog, caldera, browser, updater retain their content digests and keep running with in-memory state intact. The architecture pays off on the very next connector we add.",
      "Operator visibility: cortex-docs surfaces in /observability/connectors via the standard connector state-machine integration. Each cortex_search and cortex_xql_lookup invocation appears in the live tool-call log; the digest-pinning panel above the connectors list now shows guardian-connector-cortex-docs alongside the other 4 per-instance connector images.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Vendor-agnostic body language — skills speak data-source CLASSES (firewall, EDR, NDR, proxy, email-gateway, DNS, WAF, VPN, SaaS, cloud) and resolve to the operator's actual vendor/product at runtime via `guardian_get_technology_stack`. Same skill works on any customer stack — Palo Alto + F5 + Vectra + MDE on one deployment, CrowdStrike + Fortinet + ExtraHop + Defender on the next, no skill-body changes needed.",
      ...
      "Migration: fresh installs (v0.2.2+) ship 22 skills total in the image (24 - 12 + 10), all realistic for current xlog. Existing installs upgrading retain whatever was originally seeded in their guardian_mcp_skills volume; the 12 v0.1.33 deletions don't auto-remove from populated volumes. Operators who want a clean slate can `docker compose down -v` (drops volume — destructive) or `docker exec guardian_agent rm /app/skills/scenarios/<file>.md` for surgical removal.",
    ],
  },
  {
    version: "0.2.1",
    ...
    highlights: [
      "Closes the v0.1.34 → v0.1.36 upgrade-path gap: customers using an OLDER guardian-installer binary with `--upgrade-to 0.1.36` got every container-side v0.1.36 feature (backup/restore, sidebar reorg, connector refactor) but did NOT get the new reset-ui-password.sh recovery tool on disk because their old installer's template doesn't have the heredoc block. v0.2.1 makes that recoverable in three layered ways without forcing a fresh installer download.",
      "Layer 1 — agent image bakes the recovery tool. Dockerfile now includes `COPY installer/reset-ui-password.sh /app/tools/reset-ui-password.sh`, and a new auth-gated route `GET /api/agent/recovery/reset-ui-password` serves the bytes. Any customer who upgrades the agent image to v0.2.1+ via any path gets this endpoint, including those running `guardian-installer --upgrade-to` with their old v0.1.34 binary.",
      "Layer 2 — UI affordance. /settings/backup-restore grew a Recovery tools section with a Download reset-ui-password.sh button. One click downloads the script; the page shows the `install -m 755 reset-ui-password.sh /opt/guardian/` one-liner so operators land it correctly. No installer re-download required.",
      ...
      "Standalone release asset. release.yml now uploads reset-ui-password.sh + .sha256 as GitHub Release assets alongside the existing 4 installer artifacts. Direct wget path: `curl -L -o reset-ui-password.sh https://github.com/.../releases/download/v0.2.1/reset-ui-password.sh && sudo install -m 755 reset-ui-password.sh /opt/guardian/`",
      "v0.1.34 → v0.1.36 customers: upgrade further to v0.2.1 (any path), then visit Settings → Backup & Restore → Recovery tools → Download. One click + `sudo install -m 755 ...` lands them with a working recovery tool. No need to re-download the guardian-installer binary.",
    ],
  },
  {
    version: "0.1.36",
    ...
    highlights: [
      "Backup & Restore (`/settings/backup-restore`) — a single Download button produces guardian-backup-<timestamp>.zip carrying the deployment's complete operator-owned state: personality blob, connector instances + cleartext secrets (so the destination's GUARDIAN_SECRET_KEK can re-encrypt on restore), runtime job definitions, memory entries (without dim-bound embedding BLOBs — the destination re-embeds), all skill MD files preserving category structure, and knowledge bundle docs for reference. The Restore flow is two-step: upload zip → preview manifest + section counts via dry_run=true → click Apply. Restore order is dependency-aware: personality → instances+secrets → skills → memory → knowledge (no-op, image-baked) → jobs (last, so runtime jobs referencing connector tools have their instances enabled by then).",
      "Backup security model: cleartext secrets in the zip are intentional — the operator's GUARDIAN_SECRET_KEK doesn't need to match across deployments because the destination re-encrypts under its own KEK on write. The manifest carries an explicit warning, the UI surfaces it next to the Download button. 100 MB upload cap on restore (size check before parsing). Auth-gated via the guardian_auth cookie; the MCP-side cleartext secret read path is bearer-gated `/api/v1/instances?include_secrets=true`, mirroring v0.1.34's ProviderStore detail-endpoint pattern. Restore is upsert-or-skip by default (Personality always overwrites; everything else preserves existing entries unless ?force=true).",
  ...
  },
  {
    ...
    highlights: [
      "Architecture: the web connector flips from in-process Python module to per-instance MCP-over-HTTP container. When an operator creates a web instance via /connectors, guardian-updater pulls guardian-connector-web from GHCR and starts a container named `guardian-connector-web-<instance>` on the compose network. The agent's MCP loader generates a proxy that forwards tool calls to the container's MCP endpoint at `http://guardian-connector-web-<instance>:9000/mcp`. Customer impact: web tools behave identically; isolation gain is that one bad navigate can't leak Playwright/CDP state into the agent process anymore.",
      "Lifecycle: POST /api/v1/instances now returns runtime_style + container_start details when the connector is container-style. DELETE stops the container before removing the row. The instance store gains live `container_url` propagation — when guardian-updater starts a container it calls back to PUT /api/v1/instances/{id}/container_url and the agent automatically re-binds the proxy closures (no agent restart). 858ms reload vs the 30+ second container restart that v0.1.30's design would have required.",
      ...
      "5 bugs caught and fixed during pre-release smoke test on guardian-vm: (1) `_connector_runtime_style()` referenced `os` without importing → POST /api/v1/instances 500'd after creating the row. (2) Container proxy used `**kwargs` which FastMCP rejects → agent crashed on boot the moment any container-style instance existed. Replaced with a `compile() + exec`-synthesized proxy whose signature mirrors `connector.yaml` args (one parameter per declared arg, types mapped from yaml types to Python annotations). (3) Runtime instance_store_client SELECT'd `config`/`secret_refs` but the agent schema uses `config_json`/`secrets_json` → container crash-looped under Docker restart policy. (4) guardian-updater's HTTPS callbacks to the agent didn't honor GUARDIAN_TLS_VERIFY → CERTIFICATE_VERIFY_FAILED on every container_url callback after the v0.1.27 TLS work. Now reads the env var like the agent's own internal calls do. (5) set_container_url updated the DB but didn't trigger a tool-registry reload → cached proxy closures kept feeding `container_url=None` to tool calls until operator restarted the agent. Now invokes `reload_tools_now()` after the row update; PUT response carries new `tools_reloaded` + `tool_counts` for visibility.",
      "End-to-end smoke validated on guardian-vm: POST /api/v1/instances → 201 with container_start success → guardian-connector-web-smoke up + healthy → /health 200 → container_url propagates → tool reload re-binds → web.list_sessions through agent proxy returns `{}` → web.navigate (with `trusted: true` to bypass approval) through agent proxy → connector container → CDP → guardian-browser → real fetch of example.com returns `{url, status, title, session_id, load_time_ms}`. Full chain works.",
      "Operator-facing change: customer compose now needs guardian-updater wired to call back to the agent over the same internal URL the agent reaches itself on. The customer install kit will set `GUARDIAN_AGENT_INTERNAL_URL=https://guardian-agent:8080` + `GUARDIAN_TLS_VERIFY=0` on the updater service. Existing v0.1.30 installs without those env vars work fine for the 3 in-process connectors; web instances created on those installs hang waiting for container_url callback unless the operator adds the env vars and restarts updater.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "New images on GHCR (5 added): guardian-connector-runtime (FastMCP base + SecretStore client + audit forwarder + boot entrypoint) and the 4 per-connector images guardian-connector-{xlog,xsiam,caldera,web} that inherit from it. Operators don't need to pull or run any of these — the existing customer install path keeps working with 5 images. The new ones become useful starting v0.1.31.",
      "Schema: connector.yaml's runtimeMapping.style accepts a new `container` value alongside the existing `module` (default). Container-style connectors run as their own MCP-server containers reachable from the agent via MCP-over-HTTP; the agent's loader becomes a routing proxy. instance_store.db gains a `container_url` column (idempotent SQLite migration, mirrors the v0.1.15 enabled-column pattern) populated by guardian-updater when the container starts.",
      "Lifecycle: guardian-updater gains 4 new endpoints — POST /api/v1/connectors/{id}/instances/{name}/{start,stop,restart} + GET .../status — that operate on per-instance connector containers via Docker SDK. Image pulls retry with exponential backoff (1s→2s→4s→8s→16s, 5 attempts) and fall back to local cache for offline-deploy scenarios. Returns image_pull: 'pulled' vs 'cached' so audit can distinguish.",
      ...
      "Integration testing on guardian-vm caught two real bugs that would have hit Phase 2: (1) the runtime entrypoint's prefix-stripping logic only handled `guardian_<id>_*` and `guardian_*` prefixes — xsiam's `xsiam_*` and caldera's `caldera_*` would have advertised 73 tools with prefix still attached. Fix: 3-prefix list with `<id>_` in the middle. (2) The fallback module-scan was using `callable(val)` which matched Pydantic Request classes as if they were tools. Fix: tighten to `inspect.isfunction or iscoroutinefunction`.",
  ...
  },
  {
    version: "0.1.29",
    date: "2026-05-07",
    title: "Per-service conditional rebuild — caldera state preserved on guardian updates",
    highlights: [
      "Fix: release.yml now diffs source paths between the previous tag and the current one, then per-image either rebuilds (source changed) or retags the previous version's image with the new tag (source unchanged). Customer impact: when a release only touches guardian-agent, caldera/xlog/updater/browser images get the new version tag pointing at the SAME digest as the previous version. `docker compose pull` sees no digest change for those services, `docker compose up -d` leaves them running untouched, and stateful containers like caldera (live implants, ongoing operations) keep their state across the upgrade.",
      "Source path mapping per image: guardian-agent → mcp/agent/ + bundles/spark/; guardian-xlog → xlog/; guardian-caldera → third_party/caldera/ submodule; guardian-updater → updater/; guardian-browser → guardian-browser/. Changes outside any of these (release.yml, docs, .github, root README) trigger zero rebuilds — every service retags, costing ~30s of pull+tag+push per service vs. ~3-10min of rebuild+push.",
  ...
  },
  {
    ...
    highlights: [
      "Fix: Tool results in chat replay are no longer truncated to 500 bytes. The pre-v0.1.28 cap was a v0.1.0-era artifact from before Vertex caching existed; it directly caused the model to relay truncation stubs (\"…[result truncated, N more bytes — call X again to retrieve in full]\") back to operators instead of synthesizing answers, since calling the tool again returned the same big payload that re-truncated to the same 500 bytes. Concrete impact: `guardian_get_field_info` (~17 KB field catalog) was being clipped to 3% of its content; `guardian_get_technology_stack` (~1.5 KB) to 33%. Now retains up to 1 MiB per replayed tool result — covers every known guardian tool output with ~62x headroom.",
      "Why 1 MiB and not unlimited: this is a SAFETY VALVE for the pathological \"tool returns 100 MB blob\" case, not a budget cap. Guardian's largest known tool output is ~17 KB, well under the cap. The truncation message was rewritten too — the misleading \"call X again to retrieve in full\" hint is replaced with a clearer \"only the head is retained on replay\" so the model knows it's a replay artifact, not a recovery suggestion.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Profile-gated: `guardian-browser` does NOT auto-start with `docker compose up`. Bring it up explicitly when you want web access: `docker compose --profile browser up -d guardian-browser`. Then create a `web` connector instance via /connectors → the agent picks up `web.*` on the next refresh.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Plumbing: the bus reads origin from the existing X-Guardian-Trigger contextvar (already plumbed for audit). Chat handler now defaults trigger to `chat:<sessionId>` for interactive turns; job-triggered chats keep their existing `job:<name>` trigger. No new headers, no parallel contextvar.",
  ...
  },
  {
    ...
    highlights: [
      "Fix: `./guardian-installer --upgrade-to 0.1.21` now works. Pre-v0.1.21 the arg parser consumed the flag with `shift`, leaving `$@` empty by the time the script re-execed under sudo, so the second invocation fell back to the binary's stamped version. Customers saw 'Already at v<old> — no version change needed' even though they explicitly asked to upgrade.",
      ...
      "Bare `./guardian-installer` (no flag) was always working — the auto-bump path uses the binary's stamped version, which doesn't depend on argv. So this only affected explicit `--upgrade-to` invocations.",
      "If you're upgrading from an older installer, the binary on disk still has the bug — grab a fresh `guardian-installer` from the v0.1.21 release page, then bare-invoke (`sudo ./guardian-installer`) which auto-bumps to 0.1.21 cleanly. From v0.1.21+ forward, `--upgrade-to` will work as documented.",
    ],
  },
  {
    version: "0.1.20",
  ...
  },
  {
    ...
    highlights: [
      "Fix: guardian-updater now passes `--project-name` to its `docker compose up -d` calls — without it, compose derived the project from /host (the updater's view of the install dir) and conflicted with the actual project (`guardian`) on the container_name pin, returning a 500 to the agent.",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Root cause: agent was POSTing to /v1/services/caldera/restart on guardian-updater, but the route is /api/v1/services/caldera/restart. 404 → warning surfaced even though everything else (volume, bridge file, caldera entrypoint) was wired correctly.",
  ...
  },
  {
    ...
    highlights: [
      "`./guardian-installer` (no flag) now upgrades to whatever version the binary itself is stamped at.",
      "Older installer + newer pinned version → warning, no silent downgrade. Use --upgrade-to to force.",
      ".env missing the GUARDIAN_VERSION line is now appended (was a silent sed no-op).",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Customer install kit packs `guardian-installer` + per-version compose YAML.",
    ],
  },
];
