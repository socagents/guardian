/**
 * Per-id agent definition proxy — Round-15 / Phase S.
 *   GET    /api/agent/agent-definitions/{id} → fetch one
 *   PATCH  /api/agent/agent-definitions/{id} → partial update
 *   DELETE /api/agent/agent-definitions/{id} → remove
 */
...
async function _proxy(
  ...
  try {
    const upstream = await fetch(
      `${r.base}/api/v1/agent-definitions/${encodeURIComponent(id)}`,
      {
        method,
        headers: {
