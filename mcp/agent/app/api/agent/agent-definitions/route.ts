/**
 * Agent definitions list/upsert proxy — Round-15 / Phase S.
 */
...
export async function GET(request: Request) {
  ...
  try {
    const upstream = await fetch(
      `${r.base}/api/v1/agent-definitions${url.search}`,
      {
...
export async function POST(request: Request) {
  ...
  try {
    const upstream = await fetch(`${r.base}/api/v1/agent-definitions`, {
      method: "POST",
      headers: {
