...
const GUARDIAN_CONNECTORS: MarketplaceConnector[] = [
  // ── xsoar ───────────────────────────────────────────────────────────────
  {
    ...
    longDescription:
      "Guardian's interface to your Cortex XSOAR tenant. The chat agent monitors cases opened on XSOAR, fetches full case data, summarizes and investigates, documents findings, and updates/closes cases. The toolset spans the incident-investigation lifecycle — list and get incidents, read the War Room timeline, add entries and investigation notes, update incident fields, save and search evidence, enumerate incident types and fields, search threat indicators, create and update indicators (persist a verdict/reputation + tags back to the Threat-Intel store), and close incidents — plus SLA-aware triage (list the open incidents nearest their SLA deadline, most-overdue first), campaign discovery (read the incidents an analyst linked to a case, and search the queue for other incidents of the same type), command execution, playbook discovery (list the tenant's playbooks and read a playbook's inputs + tasks), playbook import/run, scheduled-job management (list/inspect/create/delete jobs that run a playbook on a cron), MSSP multi-tenant scoping (list XSOAR 6 child accounts + route an instance to one via the account config), XSOAR Lists, and integration troubleshooting (report each integration instance's health + last fetch error, and re-run an instance's Test). The mutating/action tools — command execution, playbook run + import, incident create/update/close, indicator create/update, scheduled-job create/delete, task completion, and List edits — are approval-gated (the operator confirms before they run); reads and routine War-Room documentation (entries, notes, evidence) are not. The connector auto-detects the platform: when an API key ID is supplied it speaks the XSOAR 8 / Cortex cloud dialect (x-xdr-auth-id header, /xsoar/public/v1 path prefix, api-<fqdn> base); otherwise it speaks XSOAR 6 on-prem (single API key in the Authorization header against the server base URL).",
    category: "Security",
    ...
    // by loadLiveMeta() below (currently 39). Keep this in step with the yaml.
    toolCount: 39,
